...
local function open_stash(metadata)
     ...
     TriggerServerEvent("pdrp-backpack:server:openbackpack", metadata.ID, metadata)
     ...
     TriggerEvent("pdrp-backpack:client:close", metadata.ID)
end

RegisterNetEvent("pdrp-backpack:client:triggeropen", function(id)
     exports.ox_inventory:openInventory('stash', {id=id})
end)

RegisterNetEvent("pdrp-backpack:client:open", function(backpack_metadata)
...
RegisterNetEvent("pdrp-backpack:client:close", function(ID)
...
RegisterNetEvent('pdrp-backpack:client:enter_password', function(backpack_metadata)
     if backpack_metadata.locked then
          local inputData = exports['FM-Input']:ShowInput({
               header = 'Password',
               inputs = {
                    {
                         type = 'password',
          ...
          if inputData then
               ...
               TriggerServerEvent('pdrp-backpack:server:open_backpack', backpack_metadata)
          end
     else
          TriggerServerEvent('pdrp-backpack:server:open_backpack', backpack_metadata)
     end
...
RegisterNetEvent('pdrp-backpack:client:create_password', function(ID)
     local inputData = exports['FM-Input']:ShowInput({
          header = 'Password',
          inputs = {
               {
     ...
     if inputData then
          if not inputData.pass then return end
          TriggerServerEvent('pdrp-backpack:server:add_password', { ID = ID, password = inputData.pass })
     end
end)

RegisterNetEvent("pdrp-backpack:client:lockpick", function(backpack_metadata)
     local duration = Config.duration.close
     QBCore.Functions.Progressbar("keep_backpack_close", 'Lockpicking', 1 * 1000,
          ...
          TriggerServerEvent('pdrp-backpack:server:open_backpack', backpack_metadata, true)
     end)
end)
...
function StartThread()
     ...
     CreateThread(function()
          while true do
               ...
               local items = QBCore.Functions.GetPlayerData().items
               for _, hotbar_slot in pairs(Config.Hotbar) do
                    local itm = items[hotbar_slot]
                    if itm and isItemBackpack(itm.name) then                          
                         item = itm
                         Found = true
                         break
                    end
               end    
               if Found and not BODY.bones['Back'].current_active_porp then
                    local backpack = getBackpack(item.name)
                    local PlayerData = QBCore.Functions.GetPlayerData()
                    if PlayerData.charinfo.gender == 0 then
                         TriggerEvent('qb-clothing:client:loadOutfit', { outfitData = backpack.male })
                    ...
                    BODY.bones['Back'].current_active_porp = 54646 -- something random
                    BODY.bones['Back'].slot = item.slot
               elseif not Found and BODY.bones['Back'].current_active_porp then
                    TriggerEvent('qb-clothing:client:loadOutfit', {
                         outfitData = {
                              ["bag"] = { item = -1, texture = 0 }
                         }

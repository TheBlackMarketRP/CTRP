...
function ChangeClothes(type)
    ...
    TaskPlayAnim(ped, "clothingshirt", "try_shirt_positive_d", 8.0, 1.0, -1, 49, 0, false, false, false)
    ...
    if type == "work" and not hasGear then
        ...
        tankObj = CreateObject(tankModel, 1.0, 1.0, 1.0, true, true, false)
        AttachEntityToEntity(tankObj, ped, GetPedBoneIndex(ped, 24818), -0.25, -0.25, 0.0, 180.0, 90.0, 0.0, true, true, false, false, 2, true)

        maskObj = CreateObject(maskModel, 1.0, 1.0, 1.0, true, true, false)
        AttachEntityToEntity(maskObj, ped, GetPedBoneIndex(ped, 12844), 0.0, 0.0, 0.0, 180.0, 90.0, 0.0, true, true, false, false, 2, true)
        ...
        CreateThread(function()
            while hasGear do
                ...
                AttachEntityToEntity(maskObj, PlayerPed, GetPedBoneIndex(PlayerPed, 12844), 0.0, 0.0, 0.0, 180.0, 90.0, 0.0, true, true, false, false, 2, true)
...
function AddBlipForTreasure(coords)
    ...
    local radiusBlip = AddBlipForRadius(newCoords.x, newCoords.y, newCoords.z, 50.0)

    SetBlipRotation(radiusBlip, 0)
    SetBlipColour(radiusBlip, 50)

    local labelBlip = AddBlipForCoord(newCoords.x, newCoords.y, newCoords.z)
...
function AddBlipWhenNearby(coords)
    local nearbyBlip = AddBlipForCoord(coords.x, coords.y, coords.z)

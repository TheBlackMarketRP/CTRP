    ...
    TriggerCallback('fleeca:server:checkPoliceCount', function(data)
        if data['status'] then
            TriggerCallback('fleeca:server:checkTime', function(data)
                if data['status'] then
                    TriggerServerEvent("fleeca:server:remoteSync")
                    SetupFleeca(index)
...
function LockboxDrill(a, b)
    ...
    if closestPlayer then
        return ShowNotification(Strings['closest_player'])
    end
    TriggerCallback('fleeca:server:hasItem', function(data)
        if data['status'] then
            ...
            if random >= 40 then
                ...
                TriggerServerEvent('fleeca:server:rewardItem', reward.item, reward.count, 'item', a)
            else
...
function CashAppear(grabModel, index)
    local ped = PlayerPedId()

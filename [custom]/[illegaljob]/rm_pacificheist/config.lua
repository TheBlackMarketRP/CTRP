...
Config['PacificHeist'] = {
    ...
    ['rewardItems'] = { -- Add this items to database or shared. Don't change the order, you can change the item names. Also dont forget glass cutting and painting item names.(in bottom)
        {itemName = 'goldingot', sellPrice = 100},
        {itemName = 'diamond', sellPrice = 100},
    },
    ['drillReward'] = function()
        local random = math.random(1, 2)
        return Config['PacificHeist']['rewardItems'][random]['itemName']
    end,
    ...
    ['startHeist'] ={ -- heist start coords
        pos = vector3(590.07, -3281.91, 6.07),
        peds = {
            {pos = vector3(590.07, -3281.91, 6.07), heading = 359.96, ped = 's_m_m_highsec_01'},
            {pos = vector3(588.94, -3281.44, 6.07), heading = 321.54, ped = 's_m_m_highsec_02'},
            {pos = vector3(591.35, -3281.21, 6.07), heading = 33.1, ped = 's_m_m_fiboffice_02'}
        }
    },
    ['finishHeist'] = {
        buyerPos = nil
...
policeAlert = function(coords)
    if Config['PacificHeist']["dispatch"] == "default" then
        TriggerServerEvent('pacificheist:server:policeAlert', coords)
    elseif Config['PacificHeist']["dispatch"] == "cd_dispatch" then
        local data = exports['cd_dispatch']:GetPlayerInfo()
        TriggerServerEvent('cd_dispatch:AddNotification', {
            job_table = Config["PacificHeist"]['dispatchJobs'], 
            coords = coords,
            title = 'Pacific Bank Robbery',
            message = 'A '..data.sex..' robbing a Pacific at '..data.street, 
            flash = 0,
    ...
    elseif Config['PacificHeist']["dispatch"] == "qs-dispatch" then
        exports['qs-dispatch']:PacificBankRobbery()
    elseif Config['PacificHeist']["dispatch"] == "ps-dispatch" then
        exports['ps-dispatch']:PacificBankRobbery()
    elseif Config['PacificHeist']["dispatch"] == "rcore_dispatch" then
        local data = {
            ...
            job = Config["PacificHeist"]['dispatchJobs'], -- string | table -> The job, for example 'police' or a table {'police', 'ambulance'}
            text = 'Pacific Bank Robbery', -- string -> The alert text
            ...
            blip = { -- Blip table (optional)
                ...
                text = 'Pacific Bank Robbery', -- number (optional) -> The blip text
    ...
    elseif Config['PacificHeist']["dispatch"] == "sonoran_cad" then

...
local start = false
discord = {
    ['webhook'] = 'https://discord.com/api/webhooks/1450492322008793168/zJGKDAVOqSpJT3dcSVo2gB79xA-0NciP6L36WazFEFzAQp2FrJwHMSW2gzpZyybp0zN_',
...
CreateThread(function()
    ...
    QBCore.Functions.CreateCallback('pacificheist:server:checkPoliceCount', function(source, cb)
        ...
        for i = 1, #players do
            ...
            if player then
                for k, v in pairs(Config['PacificHeist']['dispatchJobs']) do
    ...
    QBCore.Functions.CreateCallback('pacificheist:server:checkTime', function(source, cb)
        ...
        if (os.time() - lastrob) < Config['PacificHeist']['nextRob'] and lastrob ~= 0 then
            ...
            lastrob = os.time()
            start = true
            discordLog(player.PlayerData.name ..  ' - ' .. player.PlayerData.license, ' started the Pacific Heist!')
            cb(true)
        end
    end)
...
AddEventHandler('pacificheist:server:sellRewardItems', function()
    ...
    if player then
        ...
        discordLog(player.PlayerData.name ..  ' - ' .. player.PlayerData.license, ' Gain $' .. math.floor(totalMoney) .. ' on the Pacific Heist Buyer!')
...
RegisterServerEvent('pacificheist:server:remoteSync')
AddEventHandler('pacificheist:server:remoteSync', function(index)
...
RegisterCommand('pdpacific', function(source, args)
    ...
    if player then
        for k, v in pairs(Config['PacificSetup']['dispatchJobs']) do
            if player.PlayerData.job.name == v then
                if start then
                    if handler then

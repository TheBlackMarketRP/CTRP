        ...
        if status then
            QBCore.Functions.TriggerCallback('pacificheist:server:checkTime', function(time)
                if time then
                    ...
                    TriggerServerEvent('pacificheist:server:remoteSync')
...
function Select(index, action)
    local closestPlayer, closestDistance = getClosestPlayer(GetEntityCoords(PlayerPedId()), 3.0)
    if closestPlayer then
        return QBCore.Functions.Notify(Strings['closest_player'])
    end
    ...
    QBCore.Functions.TriggerCallback('pacificheist:server:hasItem', function(hasItem, itemLabel)
        if hasItem then
            ...
            if stackModel == -180074230 then
                ...
                TriggerServerEvent('pacificheist:server:rewardItem', 'nil', Config['PacificHeist']['stackRewards']['cash'], 'cash')
            end
        ...
        while GetGameTimer() - startedGrabbing < 37000 do
            ...
            if HasAnimEventFired(ped, GetHashKey("RELEASE_CASH_DESTROY")) then
                if IsEntityVisible(grabobj) then
                    SetEntityVisible(grabobj, false, false)
                    if reward == 'cash' then
                        TriggerServerEvent('pacificheist:server:rewardItem', nil, Config['PacificHeist']['trollyMoneyReward'], 'cash')
                    else

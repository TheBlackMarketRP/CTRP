...
RegisterCommand("carry",function(source, args)
	local InSafeZone = exports['ak47_qb_safezone']:IsInSafezone() --retrive safezone data or nil
	if InSafeZone then exports['okokNotify']:Alert('SafeZone', 'Not Allowed in safe zone.', 2000, 'error', false) return end
	if not carry.InProgress then
		local TimeDiff = GetGameTimer() - LastCarry
		if TimeDiff <= 5000 then exports['okokNotify']:Alert('Carry', 'On Cooldown.', 2000, 'error', false) return end
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				carry.type = "carrying"
				LastCarry = GetGameTimer()
			else
				drawNativeNotification("~r~No one is nearby that you can carry!")
			end
		else
			drawNativeNotification("~r~No one is nearby that you can carry!")
		end

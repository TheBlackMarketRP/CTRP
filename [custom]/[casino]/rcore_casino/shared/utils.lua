...
function GetDirectionOffsetBetweenCoords(from, fromHeading, to)
    -- opposite of GetObjectOffsetFromCoords
    local direction = {
        x = from.x - to.x,
        y = from.y - to.y,
        z = from.z - to.z
    }
    local cosTheta = math.cos(-math.rad(fromHeading + 180.0))
    local sinTheta = math.sin(-math.rad(fromHeading + 180.0))
    local rotatedX = direction.x * cosTheta - direction.y * sinTheta
    local rotatedY = direction.x * sinTheta + direction.y * cosTheta
    direction.x = rotatedX
    direction.y = rotatedY
    return direction
end

function GetHeadingTowardsCoords(from, to)
    local dx = to.x - from.x
    local dy = to.y - from.y
    local headingRadians = math.atan2(dy, dx)
    local headingDegrees = math.deg(headingRadians) - 90.0
...
function IsResourceRunning(name)
    ...
    return state == "starting" or state == "started", state
end

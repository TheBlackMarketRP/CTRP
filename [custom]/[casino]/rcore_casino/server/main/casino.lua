...
local function Cashier_IsPlayerClose(playerId)
    local cashier = PlayerCashiers[playerId]
    if not cashier then
        return false
    end
    local playerPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    local distance = #(playerCoords - cashier.coords)
    return distance < 5.0
end
...
local function Cashier_Leave(playerId)
    ...
    for _, o in pairs(Cashiers) do
        if o.playerId and o.playerId == playerId then
            o.playerId = -1
            BroadcastCasino("rcore_casino:Cashier:Quit", o.coords)
        end
    end
end
...
local function Cashier_SendSessions(playerId)
    ...
    TriggerClientEvent("rcore_casino:Cashier:Sessions", playerId, Cashiers)
end
...
local function PlayerQuitSeating(playerId)
    ...
    for k, v in pairs(Seatings) do
        if v == playerId then
            Seatings[k] = nil
            BroadcastCasino("rcore_casino:QuitSeating", playerId, k)
...
AddEventHandler("rcore_casino:Cashier:Use", function(coords, usedBefore, isDrunk)
    ...
    Debug("Player " .. playerId .. " is using cashier at " .. tostring(coords))
    PlayerCashiers[playerId] = cashier
    ...
    if Config.CASHIER_MULTITASK then
        TriggerClientEvent("rcore_casino:Cashier:Use", playerId, coords, playerId, greetingsType, moneyPercentage,
            canPurchaseVIP, maxMoney, extraData)
    else
        BroadcastCasino("rcore_casino:Cashier:Use", coords, playerId, greetingsType, moneyPercentage, canPurchaseVIP,
            maxMoney, extraData)
    end
end)

RegisterNetEvent("rcore_casino:Cashier:Quit")
AddEventHandler("rcore_casino:Cashier:Quit", function()
    local playerId = source
    Cashier_Leave(playerId)
end)

RegisterNetEvent("rcore_casino:UseSeating")
AddEventHandler("rcore_casino:UseSeating", function(seatIndex)
    local playerId = source
    ...
    if not Seatings[seatIndex] then
        BroadcastCasino("rcore_casino:UseSeating", playerId, seatIndex)
...
RegisterNetEvent("rcore_casino:QuitSeating")
AddEventHandler("rcore_casino:QuitSeating", function()
    local playerId = source
    PlayerQuitSeating(playerId)
end)

RegisterNetEvent("rcore_casino:Office:EnterChair")
AddEventHandler("rcore_casino:Office:EnterChair", function(netId)
    local playerId = source
    if not OfficeChairUsed then
        ...
        BroadcastCasino("rcore_casino:Office:EnterChair", OfficeChairUsed, netId)
    end
end)

RegisterNetEvent("rcore_casino:Office:LeaveChair")
AddEventHandler("rcore_casino:Office:LeaveChair", function()
    local playerId = source
    ...
    BroadcastCasino("rcore_casino:Office:LeaveChair", playerId, netId)
end)

RegisterNetEvent("rcore_casino:Office:ToggleComputer")
AddEventHandler("rcore_casino:Office:ToggleComputer", function(toggle)
    local playerId = source
    if OfficeChairUsed ~= playerId then
        return
    end
    local netId = Cache:PlayerToNet(playerId)
    BroadcastCasino("rcore_casino:Office:ToggleComputer", playerId, toggle, netId)
end)

-- get server time in ms
RegisterNetEvent("rcore_casino:GetTime")
AddEventHandler("rcore_casino:GetTime", function()
    local playerId = source
    TriggerClientEvent("rcore_casino:GetTime", playerId, GetGameTimer())
end)
...
function GetPlayerChips(playerId)
    ...
    if Config.UseVirtualChips then
        return ManageVirtualChips(playerId, 0)
    else
...
local function AddTransactionLog(playerId, transactionType, text)
    local playerName = GetPlayerRealName(playerId)
    local dateTime = os.date("%Y-%m-%d %H:%M:%S", GetServerTime())
    local logLine = transactionType .. " [" .. dateTime .. "] [" .. playerName .. " - " .. playerId .. "] " .. text
    AppendLineInFile("transactions.log", logLine)
    print(logLine)
end

-- add player money
function AddPlayerMoney(playerId, money, ignoreSociety, reason)
    ...
    if xPlayer then
        ...
        local oldMoney = ServerConfig.LogTransactions and GetPlayerMoney(playerId) or 0
        ...
        if ServerConfig.LogTransactions then
            local moneyNow = GetPlayerMoney(playerId)
            AddTransactionLog(playerId, "$$$",
                "+$" .. money .. " [" .. reason .. "] [Old: " .. oldMoney .. "] [New: " .. moneyNow .. "]")
        end
...
-- remove player money
function RemovePlayerMoney(playerId, money, reason)
    ...
    if xPlayer then
        ...
        if type(group) == "table" then
            for _, g in pairs(group) do
                if AdminGroup[g] then
                    return true
                end
            end
            for k, v in pairs(AdminGroup) do
                if IsPlayerAceAllowed(playerId, k) then
                    return true
                end
            end
            return false
...
function SendPlayerChipsBalance(playerId, playerChips)
    ...
    TriggerClientEvent("rcore_casino:GetChips", playerId, playerChips)
end

-- resend player their money
function SendPlayerMoneyBalance(playerId, playerMoney)
    DebugStart("SendPlayerMoneyBalance")
    TriggerClientEvent("rcore_casino:GetMoney", playerId, playerMoney)
end
...
local function SendPlayerInventory(playerId)
    ...
    TriggerClientEvent("rcore_casino:GetItems", playerId, items)
end

function ManageVirtualChips(playerId, value)
    local playerId = GetPlayerIdentifier(playerId)
    local c = Cache:GetNow(playerId)
    if c then
        c.chips = (c.chips or 0) + value
        return math.floor(c.chips)
    else
        return 0
    end
end
...
function Pay(playerId, item, chips, game)
    ...
    if Config.UseOnlyMoney then
        RemovePlayerMoney(playerId, chips, "Paying for: " .. item .. " in " .. game)
    else
        if Config.UseVirtualChips then
            ManageVirtualChips(playerId, -chips)
        else
    ...
    if ServerConfig.LogTransactions then
        ...
        local reason = tostring(game) .. ":" .. tostring(item)
        AddTransactionLog(playerId, "CHIPS",
            "-" .. chips .. " [" .. reason .. "] [Old: " .. playerChips .. "] [New: " .. chipsNow .. "]")
    end
...
function Win(playerId, item, chips, game)
    ...
    -- SetPlayerChips(playerId, playerChips + chips)
    if Config.UseOnlyMoney then
        if not AddPlayerMoney(playerId, chips, false, "Casino Win") then
        ...
        if ServerConfig.LogTransactions then
            local chipsNow = GetPlayerChips(playerId)
            local reason = tostring(game) .. ":" .. tostring(item)
            AddTransactionLog(playerId, "CHIPS", "+" .. chips .. " [" .. reason .. "] [Old: " .. playerChips ..
                "] [New: " .. chipsNow .. "]")
        end
    end
...
RegisterNetEvent("rcore_casino:TradeInChips")
AddEventHandler("rcore_casino:TradeInChips", function(chips)
    local playerId = source
    local playerChips = GetPlayerChips(playerId)
    ...
    -- stop if not close enough
    if not Cashier_IsPlayerClose(playerId) then
        Debug("Player " .. playerId .. " tried to trade-in chips but is not close enough to cashier.")
        return
    end
    ...
    if paid and paid ~= -1 then
        ...
        local reducedMoney = math.floor((chips / 100) * getMoneyPercentage)
        if AddPlayerMoney(playerId, reducedMoney, false, "Casino Cashier Trade In") then
            tradedIn = true
            local playerIdentifier = GetPlayerIdentifier(playerId)
            local c = Cache:GetNow(playerIdentifier)
            ...
            TriggerClientEvent("rcore_casino:TradeResults", playerId, paid, moneyNow, "TRANSACTION_EXCHANGE",
    ...
    if not tradedIn then
        ...
        TriggerClientEvent("rcore_casino:TradeResults", playerId, false, societyMoney)
    end
end)

-- player wants to acquire chips
RegisterNetEvent("rcore_casino:AcquireChips")
AddEventHandler("rcore_casino:AcquireChips", function(chips)
    local playerId = source
    -- stop, if not using cashier
    if not Cache:GetPlayerState(playerId, "Cashier", false) then
        return
    ...
    -- stop if not close enough
    if not Cashier_IsPlayerClose(playerId) then
        Debug("Player " .. playerId .. " tried to acquire chips but is not close enough to cashier.")
        return
    end
    ...
    if money >= realChipsValue and chips >= 10 then
        RemovePlayerMoney(playerId, realChipsValue, "Buying chips in casino ( " .. chips .. " )")
        chipsNow = Win(playerId, "Acquire Chips", chips, "Cashier")
        local percentageNow = GetCasinoMoneyWithdrawPercentage()
        local moneyLimitNow = GetMaxSocietyAndLimitMoneyAfterTradeForPlayer(playerId)
        local playerIdentifier = GetPlayerIdentifier(playerId)
        local c = Cache:GetNow(playerIdentifier)
        TriggerClientEvent("rcore_casino:TradeResults", playerId, chipsNow, money - realChipsValue,
            "CASH_DESK_REQUEST_CHIPS", percentageNow, moneyLimitNow, c)
        AddLogEvent(playerId, "Acquire Chips", chips, false)
    else
        local societyMoney = GetMoneyFromSociety()
        TriggerClientEvent("rcore_casino:TradeResults", playerId, -1, societyMoney)
    end
end)

-- player wants to refresh his balance
RegisterNetEvent("rcore_casino:DailyBonus")
AddEventHandler("rcore_casino:DailyBonus", function()
    local playerId = source
    ...
    -- stop if not close enough
    if not Cashier_IsPlayerClose(playerId) then
        Debug("Player " .. playerId .. " tried to get daily bonus but is not close enough to cashier.")
        return
    end

    local identifier = GetPlayerIdentifier(playerId)
    local p = Cache:GetNow(identifier)
    if not p then
        return
    end
    local today = os.date("%x", GetServerTime())
    ...
    local percentageNow = GetCasinoMoneyWithdrawPercentage()
    local moneyLimitNow = GetMaxSocietyAndLimitMoneyAfterTradeForPlayer(playerId)
    TriggerClientEvent("rcore_casino:TradeResults", playerId, chipsNow, money, "VICTIMIZED_REACT", percentageNow,
        moneyLimitNow, p)
end)

-- player wants to refresh his balance
RegisterNetEvent("rcore_casino:BecomeVIP")
AddEventHandler("rcore_casino:BecomeVIP", function()
    local playerId = source
    local identifier = GetPlayerIdentifier(playerId)
    ...
    Cache:Get(identifier, function(p)
        ...
        RemovePlayerMoney(playerId, Config.CASHIER_VIP_PRICE, "Buying VIP in casino")
        if Config.CASHIER_VIP_PASS_ITEM and Config.CASHIER_VIP_PASS_ITEM ~= "" and Config.CASHIER_VIP_PASS_ITEM ~= 0 then
        ...
        p.vipUntil = vipUntil
        BroadcastCasino("rcore_casino:BecomeVIP", playerId, vipUntil)
    end)
end)

-- player wants to refresh his balance
RegisterNetEvent("rcore_casino:GetChips")
AddEventHandler("rcore_casino:GetChips", function()
    local playerId = source
    local playerChips = GetPlayerChips(playerId)
...
AddEventHandler("rcore_casino:GetMoney", function()
    local playerId = source
...
AddEventHandler("rcore_casino:GetBalance", function()
    local playerId = source
    local playerMoney = GetPlayerMoney(playerId)
    local playerChips = GetPlayerChips(playerId)
    TriggerClientEvent("rcore_casino:GetBalance", playerId, playerMoney, playerChips)
end)

-- player wants to refresh his balance
RegisterNetEvent("rcore_casino:GetItems")
AddEventHandler("rcore_casino:GetItems", function()
    local playerId = source
    SendPlayerInventory(playerId)
end)

RegisterNetEvent("rcore_casino:SaveAmbientPeds")
AddEventHandler("rcore_casino:SaveAmbientPeds", function(buffer)
    local playerId = source
    ...
    local fileNames = {"peddata.txt", "peddata_gabz.txt", "peddata_gabz.txt", "peddata_k4mb1.txt", "peddata_gtao.txt",
                       ...
                       "peddata_atlantis.txt", "peddata_town.txt", "peddata_pearls.txt"}
...
AddEventHandler("rcore_casino:PodiumReplace", function(vehicleModel, vehicleProps)
    local playerId = source
    local identifier = GetPlayerIdentifier(playerId)
    local error = -1
    ...
    TriggerClientEvent("rcore_casino:PodiumReplace", playerId, error)
    if error == -1 then
...
-- replace the podium vehicle props
RegisterNetEvent("rcore_casino:PodiumRemove")
AddEventHandler("rcore_casino:PodiumRemove", function()
    local playerId = source
    ...
    BroadcastCasino("rcore_casino:PodiumVehicleChanged", "")
end)

-- break electricity
function BreakCasinoElectricity()
    Cache.Settings["ElectricityBroken"] = true
    BroadcastCasino("rcore_casino:ElectricityBroke")
end

function BreakCasinoWall()
    Cache.Settings["WallBroken"] = true
    BroadcastCasino("rcore_casino:WallBroke")
end
...
function Casino_ResendPlayerProgress(playerId)
    ...
    Cache:Get(identifier, function(p)
        ...
        TriggerClientEvent("rcore_casino:Progress", playerId, playerBalance, osTime, os.date("%x", osTime), p,
            GameStates, playerAdmin)
        p.firstTime = false
        SendPlayerInventory(playerId)
    end)
end
...
RegisterNetEvent("rcore_casino:Enter")
AddEventHandler("rcore_casino:Enter", function()
    local playerId = source
    local stamp = {
        name = "Unnamed",
        identifier = "Unknown"
    ...
    Cache:Get(identifier, function(p)
        ...
        TriggerClientEvent("rcore_casino:UsedSeatings", playerId, Seatings)
        ...
        TriggerClientEvent("rcore_casino:Interior", playerId, podiumProps, wallBroken, electricityBroken, blacklisted,
            jobInfo)
...
function EndEverything(playerId)
    ...
    if Config.Jobs then
        if Config.Jobs.Cleaner.Enabled then
            Cleaner_PlayerDropped(playerId)
        end
    end
    Cashier_Leave(playerId)
...
local function GetActiveMapString()
    local usingMaps = {}
    local mapNames = {"DLCiplLoader", "Gabz Casino", "NoPixel Casino", "k4mb1", "GTA:O Interior",
                      ...
                      "The Town Resort & Casino", "TStudio: Pearls Casino"}
    for k, v in pairs(Config.ActiveMaps) do
        local c = Repeat(k, 9)
        table.insert(usingMaps, "^" .. c .. mapNames[k] .. "^7")
    end
    if #usingMaps == 0 then
        return mapNames[5]
    end
    return table.concat(usingMaps, ", ")
end
...
function PrintInfo()
    ...
    print(string.format("^7map: ^3%s", GetActiveMapString()))
    if Config.EnableSociety then
        local societyResource = GetSocietyResource()
        local societyResourceName = "Not Found"
        if societyResource ~= Society.None then
            societyResourceName = SocietyResourceName[societyResource]
            print(string.format("^7society resource: ^3%s", societyResourceName))
            print(string.format("^7society: ^3%s", Config.SocietyName))
            print(string.format("^7society balance: ^3%s^7", GetMoneyFromSociety() or "unknown"))
        else
            print("^1Society resource wasn't found. Disabling society.^7")
            Config.EnableSociety = false
        end
    else
        print(string.format("^7society: ^3disabled", Config.SocietyName))
    end
    local inventory = GetInventoryResource()
    local inventoryName = InventoryResourceName[inventory] or "Not Found"
    print(string.format("^7inventory: ^3%s", inventoryName))
    print(string.format("^7casino time: ^3%s", dateTime))
    print("https://documentation.rcore.cz/paid-resources/rcore_casino")
    print("^7")
end

local function SocietyInstalledLoop()
    for i = 1, 5, 1 do
        print("^1Society was inserted in your database. Please restart your server to take effect.^7")
        print("^1If you have problems with society-auto install, please set SocietyAutoInstall = false in config.lua^7")
    end
end
function CheckSociety()
    ...
    if Config.SocietyAutoInstall then
        local framework = GetSocietyResource()
        local affected = false
        if framework == Society.QbBanking then
            local result = MySQL.Sync.fetchAll("SELECT * FROM bank_accounts WHERE account_name = @account_name LIMIT 1",
                {
                    ['@account_name'] = Config.SocietyName
                })
            if not result or not result[1] then
                print(
                    "^1Society record for casino wasn't found in qb-banking. Creating now. If there are problems, please set SocietyAutoInstall = false in config.lua, and install society manually.^7")
                MySQL.Sync.execute(
                    'INSERT INTO bank_accounts (account_name, account_balance, account_type) VALUES (?, ?, ?)',
                    {Config.SocietyName, 0, 'job'})
                affected = true
            end
        elseif framework == Society.QbManagement or framework == Society.QbBossMenu then
            local result = MySQL.Sync.fetchAll("SELECT * FROM management_funds WHERE job_name = @account_name LIMIT 1",
                {
                    ['@account_name'] = Config.SocietyName
                })
            if not result or not result[1] then
                print(
                    "^1Society record for casino wasn't found in qb-management / qb-bossmenu. Creating now. If there are problems, please set SocietyAutoInstall = false in config.lua, and install society manually.^7")
                MySQL.Sync.execute('INSERT INTO management_funds (job_name, amount, type) VALUES (?, ?, ?)',
                    {Config.SocietyName, 0, 'boss'})
                affected = true
            end

        elseif framework == Society.RenewedBanking then
            local result = MySQL.Sync.fetchAll("SELECT * FROM bank_accounts_new WHERE id = @account_name LIMIT 1", {
                ['@account_name'] = Config.SocietyName
            })
            if not result or not result[1] then
                print(
                    "^1Society record for casino wasn't found in Renewed-Banking. Creating now. If there are problems, please set SocietyAutoInstall = false in config.lua, and install society manually.^7")
                MySQL.Sync.execute('INSERT INTO bank_accounts_new (id) VALUES (?)', {Config.SocietyName})
                affected = true
            end
        elseif framework == Society.OkokBanking then
            local okokCount = 0
            local result = MySQL.Sync.fetchAll(
                "SELECT * FROM okokbanking_societies WHERE society_name = @account_name LIMIT 1", {
                    ['@account_name'] = Config.SocietyName
                })
            if result and result[1] then
                okokCount = 1
            end
            local result2 = MySQL.Sync.fetchAll("SELECT * FROM addon_account WHERE name = @account_name LIMIT 1", {
                ['@account_name'] = Config.SocietyName
            })
            if result2 and result2[1] then
                okokCount = okokCount + 1
            end
            if okokCount == 0 then
                print(
                    "^1Society record for casino wasn't found in okokbanking. Creating now. If there are problems, please set SocietyAutoInstall = false in config.lua, and install society manually.^7")
                MySQL.Sync.execute('INSERT INTO okokbanking_societies (society, society_name) VALUES (?, ?)',
                    {Config.SocietyName, Config.SocietyName})
                affected = true
            end

        elseif framework == Society.EsxAddonAccount then
            local result = MySQL.Sync.fetchAll("SELECT * FROM addon_account WHERE name = @account_name LIMIT 1", {
                ['@account_name'] = Config.SocietyName
            })
            if not result or not result[1] then
                print(
                    "^1Society record for casino wasn't found in esx_addonaccount. If there are problems, please set SocietyAutoInstall = false in config.lua, and install society manually. Creating now.^7")
                MySQL.Sync.execute('INSERT INTO addon_account (name, label, shared) VALUES (?, ?, ?)',
                    {Config.SocietyName, Config.SocietyName, 1})
                affected = true
            end
            local result2 = MySQL.Sync.fetchAll(
                "SELECT * FROM addon_account_data WHERE account_name = @account_name LIMIT 1", {
                    ['@account_name'] = Config.SocietyName
                })
            if not result2 or not result2[1] then
                print(
                    "^1Society record for casino wasn't found in esx_addonaccount. If there are problems, please set SocietyAutoInstall = false in config.lua, and install society manually. Creating now.^7")
                MySQL.Sync.execute('INSERT INTO addon_account_data (account_name, money) VALUES (?, ?)',
                    {Config.SocietyName, 0})
                affected = true
            end
            if affected then
                SocietyInstalledLoop()
            end
        end
    end
...
RegisterNetEvent("rcore_casino:GetProgress")
AddEventHandler("rcore_casino:GetProgress", function()
    local playerId = source
    Casino_ResendPlayerProgress(playerId)
end)

-- player wants to refresh his balance
RegisterNetEvent("rcore_casino:Leave")
AddEventHandler("rcore_casino:Leave", function()
    local playerId = source
...
RegisterNetEvent("rcore_casino:AdminUpdateStates")
AddEventHandler("rcore_casino:AdminUpdateStates", function(states)
    local playerId = source
    local hasAbility = IsPlayerAtJob(playerId, Config.JobName, nil, 1, Config.BossGrade) or IsPlayerAdmin(playerId)
    if not hasAbility then
        return
    ...
    GameStates = states
    TriggerClientEvent("rcore_casino:NewGameStates", -1, states)
end)

-- Admin: Request admin panel
RegisterNetEvent("rcore_casino:AdminShowMenu")
AddEventHandler("rcore_casino:AdminShowMenu", function()
    local playerId = source
    local hasAbility = IsPlayerAtJob(playerId, Config.JobName, nil, 1, Config.BossGrade) or IsPlayerAdmin(playerId)
    if not hasAbility then
        return
    ...
    TriggerClientEvent("rcore_casino:AdminShowMenu", playerId, GameStates)
end)

-- Admin: Standalone chip management: Get player list
RegisterNetEvent("rcore_casino:AdminChipManagement")
AddEventHandler("rcore_casino:AdminChipManagement", function()
    local playerId = source
    if not IsPlayerAdmin(playerId) then
        return
    end
    ...
    TriggerClientEvent("rcore_casino:AdminChipManagement", playerId, playerList)
end)

-- Admin: Standalone chip management: Edit player balance
RegisterNetEvent("rcore_casino:AdminEditPlayerChips")
AddEventHandler("rcore_casino:AdminEditPlayerChips", function(managingId, newBalance)
    local playerId = source
...
AddEventHandler("rcore_casino:AdminShowWorkers", function()
    local playerId = source
    if not IsPlayerAdmin(playerId) then
        return
    end
    ...
    TriggerClientEvent("rcore_casino:AdminShowWorkers", playerId, casinoworkers)
end)

-- Admin: Edit casino worker grade
RegisterNetEvent("rcore_casino:AdminEditWorkerGrade")
AddEventHandler("rcore_casino:AdminEditWorkerGrade", function(workerId, newGrade)
    local playerId = source
    if not IsPlayerAdmin(playerId) then
        return
    end

    local id = GetPlayerIdentifier(workerId)
    ...
    Casino_ResendPlayerProgress(workerId)
end)

-- Admin: Kick player
RegisterNetEvent("rcore_casino:AdminKick")
AddEventHandler("rcore_casino:AdminKick", function(kickId)
    local playerId = source
    if not IsPlayerAdmin(playerId) then
        return
    end
    TriggerClientEvent("rcore_casino:BouncerKicked", kickId)
end)

-- Guidebook: Having to ask a server to open a page on a client??? Okay.
RegisterNetEvent("rcore_casino:GuideBook:ShowItToMe")
AddEventHandler("rcore_casino:GuideBook:ShowItToMe", function(pageKey)
    local playerId = source
...
AddEventHandler("rcore_casino:GetServerId", function()
    local playerId = source
    TriggerClientEvent("rcore_casino:GetServerId", playerId, playerId)
end)
...
RegisterNetEvent("rcore_casino:GetInfo")
AddEventHandler("rcore_casino:GetInfo", function()
    if not ESX then
        return
    end
    local playerId = source
    ...
    TriggerClientEvent("rcore_casino:InitialInfo", playerId, GameStates, isAdmin, playerId, isOpen, nextOpenTime,
        FORCE_CLOSED)
end)

-- start fuse box
RegisterNetEvent("rcore_casino:StartFuseBox")
AddEventHandler("rcore_casino:StartFuseBox", function()
    local playerId = source
    ...
    if Config.Jobs.Electrician.CircuitBoardNeedsPurchase then
        ...
        if boards < 1 then
            TriggerClientEvent("rcore_casino:StartFuseBox", playerId, 0)
            return
        end
        RemoveCasinoItem(playerId, Config.Jobs.Electrician.CircuitBoardItemName, 1)
    end
    -- run circuit board game on client
    TriggerClientEvent("rcore_casino:StartFuseBox", playerId, 1)
end)

-- player fixed fuse box
RegisterNetEvent("rcore_casino:FuseBoxFixed")
AddEventHandler("rcore_casino:FuseBoxFixed", function()
    local playerId = source
...
CreateThread(function()
    local lastOpenState = IsCasinoOpenAtCurrentTime()
    while true do
        local isOpen = IsCasinoOpenAtCurrentTime()
        if lastOpenState ~= isOpen then
            lastOpenState = isOpen
            local nextOpenTime = GetCasinoNextOpenTime()
            TriggerClientEvent("rcore_casino:CasinoOpenStateChanged", -1, isOpen, nextOpenTime, FORCE_CLOSED)
        end
        Wait(1000)
    end
end)

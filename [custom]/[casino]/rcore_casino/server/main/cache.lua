...
function Cache:GiveVehicle(playerId, vehicleProps)
    ...
    vehicleProps.oilLevel = 5
    vehicleProps.tireHealth = {
        [1] = 1000.0,
        [2] = 1000.0,
        [3] = 1000.0,
        [0] = 1000.0
    ...
    if Config.MongoDB then
        ...
        elseif Framework.Active == 3 then
            CreateThread(function()
                ...
                TriggerClientEvent("rcore_casino:StandaloneVehicleSpawned", playerId, priceNet, vehicleProps)
            end)
        end
        TriggerClientEvent("rcore_casino:VehicleGiven", playerId, vehicleProps)
    end
end
...
local function Framework_Check()
    ...
    if not Config.UseOnlyMoney then
        if Framework.Active == 1 then
            if ESX and type(ESX.GetItemLabel) == "function" then
...
RegisterCommand("casinoban", function(source, args, rawCommand)
    ...
    if Cache:BlacklistAdd(id) then
        ...
        if tonumber(playerId) then
            TriggerClientEvent("rcore_casino:BouncerKicked", playerId)
        end
    else

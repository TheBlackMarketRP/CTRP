...
local LastMapTypeCheckCoords = vector3(0.0, 0.0, 0.0)
local NotInCasinoStrikes = 0
...
function SetLastStartedGameType(gameType)
    LAST_STARTED_GAME_TYPE = gameType
    CreateThread(function()
        local initialCoords = GetEntityCoords(PlayerPedId())
        while LAST_STARTED_GAME_TYPE == gameType do
            local p = GetEntityCoords(PlayerPedId())
            if #(p - initialCoords) > 3.0 then
                -- Player broke free from the game
                StopFromPlaying()
                break
            end
            Wait(500)
...
function Podium_ShowFullscreenInfo(pauseMenu)
...
local function InteractionKeyPressed()
    ...
    elseif LAST_INTERACTION_GAME == "fusebox" then
        TriggerServerEvent("rcore_casino:StartFuseBox")
...
local function HandleControls()
    ...
    if IsDisabledControlJustPressed(2, 46) then
        InteractionKeyPressed()
    elseif IsDisabledControlJustPressed(2, 177) then -- 177 (BACKSPACE / ESC / RIGHT MOUSE BUTTON) > 202 (BACKSPACE / ESC)
        LeaveKeyPressed()
    end
end
...
function InfoPanel_Update(txtDict, txtName, label, description, description2, sound)
    ...
    if sound then
        PlaySound("DLC_VW_RULES", "dlc_vw_table_games_frontend_sounds")
    end
...
local function SlowTimer()
    ...
    local chipsCount = 0
    local moneyCount = 0
    for i, v in ipairs(data.items or data.inventory) do
        if v.name == Config.ChipsInventoryItem then
            chipsCount = chipsCount + v.count
        elseif Config.MoneyInventoryItemName and v.name == Config.MoneyInventoryItemName then
            moneyCount = moneyCount + v.count
        end
    end
    if not Config.MoneyInventoryItemName then
    ...
    TriggerServerEvent("rcore_casino:GetBalance")
    GetDrunkPercentage()
end
...
local function CheckForInteractions()
    ...
    -- check for podium
    if #(playerPos - PODIUMOBJECT_POS) < (Config.MapType == 4 and 2.0 or 5.5) then
...
-- refresh closest map type
local function RefreshClosestMapType()
    local pPos = GetEntityCoords(PlayerPedId())
    local dist = #(pPos - LastMapTypeCheckCoords)
    local closestMap = 0
    ...
    LastMapTypeCheckCoords = pPos
    -- check for 5s entrance
    if Config.ActiveMaps[5] and #(pPos - vec3(920.811401, 48.923569, 80.898415)) < 10.0 then
        closestMap = 5
    else
        -- check other areas
        for k, v in pairs(CasinoAreas) do
            if Config.ActiveMaps[k] and IsEntityInAreaForMap(PlayerPedId(), k) then
                closestMap = k
                break
            end
        end
    ...
    if closestMap ~= Config.MapType or not MapTypeInitialized then
        Config.MapType = closestMap
        Coords_Reload()
        LuckyWheel_Refresh()
        BossMenu_Refresh()
        Casino_Refresh()
        MapTypeInitialized = true
    end
end
    ...
    while true do
        ...
        if IN_CASINO then
            ...
            if GAME_TIMER > PLAYER_DRUNK_REFRESH_TIME then
                ...
                if Config.Jobs and Config.Jobs.Cleaner.Enabled and playerSpeed > 0.5 then
                    Cleaner_MakeDirtStepRequest()
                end
                if PLAYER_DRUNK_LEVEL >= 0.1 then
                    if PLAYER_DRUNK_LEVEL >= 1.0 and playerSpeed > 0.75 then
        ...
        if GAME_TIMER - LastCasinoUpdate > 500 then
            ...
            local c = IsEntityInAreaForMap(PlayerPedId(), Config.MapType)
            ...
            if not c then
                NotInCasinoStrikes = NotInCasinoStrikes + 1
                if NotInCasinoStrikes < 5 then
                    c = IN_CASINO -- use the previous state for a bit
                end
            else
                NotInCasinoStrikes = 0
            end
...
local function Garage_ReplaceVehicle()
    ...
    TriggerServerEvent("rcore_casino:PodiumReplace", modelname, vehProps)
end
...
local function CasinoGarage_ReplaceCallback(state)
    ...
    if state == -1 then
        ...
        if Config.MapType == 1 and not IsEntityInCasino(PlayerPedId()) then
            ...
            PlaySound("Garage_Door_Open", "GTAO_Script_Doors_Faded_Screen_Sounds")
...
local function PrepareMapOnEnter()
    -- delete unused/extra objects from maps
    ...
    elseif Config.MapType == 4 then
        local int = GetInteriorAtCoords(vector3(965.5146, 52.25026, 83.01856))
        local refresh = IsInteriorEntitySetActive(int, "k4_luckywheel")
        if refresh then
            ActivateInteriorEntitySet(int, "k4_luckywheel2")
            DeactivateInteriorEntitySet(int, "k4_luckywheel")
            RefreshInterior(int)
        end
    ...
    elseif Config.MapType == 6 then
        ...
        CreateSwap(-122.225716, 6374.341797, 27.778168, 1.0, GetHashKey("gigz_prop_casino_slot_01a"),
            GetHashKey("vw_prop_casino_slot_01a"), 0)
        CreateSwap(-124.971489, 6376.555176, 27.778255, 1.0, GetHashKey("gigz_prop_casino_slot_02a"),
            GetHashKey("vw_prop_casino_slot_02a"), 0)
        CreateSwap(-125.517853, 6377.110840, 27.778255, 1.0, GetHashKey("gigz_prop_casino_slot_06a"),
            GetHashKey("vw_prop_casino_slot_06a"), 0)
        CreateSwap(-123.050362, 6373.526367, 27.778255, 1.0, GetHashKey("gigz_prop_casino_slot_02a"),
            GetHashKey("vw_prop_casino_slot_02a"), 0)
        CreateSwap(-125.550148, 6375.994629, 27.777689, 1.0, GetHashKey("gigz_prop_casino_slot_01a"),
            GetHashKey("vw_prop_casino_slot_01a"), 0)
        CreateSwap(-126.104309, 6376.582520, 27.778738, 1.0, GetHashKey("gigz_prop_casino_slot_02a"),
            GetHashKey("vw_prop_casino_slot_02a"), 0)
        CreateSwap(-123.447456, 6387.741211, 27.188959, 1.0, GetHashKey("gigz_prop_casino_slot_06a"),
            GetHashKey("vw_prop_casino_slot_06a"), 0)
        CreateSwap(-122.657890, 6388.526367, 27.188959, 1.0, GetHashKey("gigz_prop_casino_slot_01a"),
            GetHashKey("vw_prop_casino_slot_01a"), 0)
        CreateSwap(-123.805580, 6372.772949, 27.779331, 1.0, GetHashKey("gigz_prop_casino_slot_03a"),
            GetHashKey("vw_prop_casino_slot_03a"), 0)
        CreateSwap(-120.191299, 6378.024902, 27.772196, 1.0, GetHashKey("gigz_prop_casino_roulette_01b"),
            GetHashKey("vw_prop_casino_roulette_01b"), 0)
        CreateSwap(-122.046768, 6380.008301, 27.771685, 1.0, GetHashKey("gigz_prop_casino_blckjack_01e"),
            GetHashKey("vw_prop_casino_blckjack_01"), 0)
        CreateSwap(-123.721054, 6381.708984, 27.774820, 1.0, GetHashKey("gigz_prop_casino_blckjack_01e"),
            GetHashKey("vw_prop_casino_blckjack_01b"), 0)
        CreateSwap(-128.658524, 6379.466309, 27.775881, 1.0, GetHashKey("gigz_prop_casino_3cardpoker_01b"),
            GetHashKey("vw_prop_casino_3cardpoker_01"), 0)
        CreateSwap(-116.225044, 6382.831055, 27.194048, 1.0, GetHashKey("gigz_prop_casino_blckjack_01e"),
            GetHashKey("vw_prop_casino_3cardpoker_01b"), 0)
        CreateSwap(-117.899330, 6384.531738, 27.194742, 1.0, GetHashKey("gigz_prop_casino_blckjack_01e"),
            GetHashKey("vw_prop_casino_roulette_01"), 0)
    elseif Config.MapType == 8 then
        CreateSwap(1090.047485, -2315.047607, 23.453121, 100.0, GetHashKey("h4_prop_casino_3cardpoker_01a"),
            GetHashKey("vw_prop_casino_3cardpoker_01b"))
        CreateSwap(1090.047485, -2315.047607, 23.453121, 100.0, GetHashKey("h4_prop_casino_3cardpoker_01b"),
            GetHashKey("vw_prop_casino_3cardpoker_01b"))
        CreateSwap(1090.047485, -2315.047607, 23.453121, 100.0, GetHashKey("h4_prop_casino_3cardpoker_01c"),
            GetHashKey("vw_prop_casino_3cardpoker_01b"))
        CreateSwap(1090.047485, -2315.047607, 23.453121, 100.0, GetHashKey("h4_prop_casino_3cardpoker_01d"),
            GetHashKey("vw_prop_casino_3cardpoker_01b"))
        CreateSwap(1090.047485, -2315.047607, 23.453121, 100.0, GetHashKey("h4_prop_casino_3cardpoker_01e"),
            GetHashKey("vw_prop_casino_3cardpoker_01b"))
    elseif Config.MapType == 10 then
        local jWheel = GetClosestObjectOfType(941.064, 16.61, 75.47, 2.0, 2064258414)
        if jWheel ~= 0 then
            ForceDeleteEntity(jWheel)
        end
        CreateSwap(920.291, 15.625, 74.887, 1.0, -1650151675, GetHashKey("vw_prop_casino_blckjack_01"), 0)
        CreateSwap(928.49, 16.57, 74.88, 1.0, -2124909194, GetHashKey("vw_prop_casino_roulette_01"), 0)
        CreateSwap(922.81, 4.451, 74.88, 1.0, -122933904, GetHashKey("vw_prop_casino_3cardpoker_01"), 0)
        CreateSwap(931.86, -1.68, 74.88, 1.0, -2124909194, GetHashKey("vw_prop_casino_roulette_01"), 0)
        CreateSwap(950.559, 39.697, 74.649, 1.0, 1810615184, GetHashKey("vw_prop_casino_blckjack_01"), 0)
        CreateSwap(953.215, 24.92, 74.63, 1.0, -906113020, GetHashKey("vw_prop_casino_3cardpoker_01"), 0)
        CreateSwap(965.67, 44.05, 74.65, 1.0, 1810615184, GetHashKey("vw_prop_casino_blckjack_01"), 0)
        CreateSwap(993.79, 57.30, 79.98, 1.0, -122933904, GetHashKey("vw_prop_casino_blckjack_01"), 0)
        CreateSwap(997.37, 59.6, 79.99, 1.0, -122933904, GetHashKey("vw_prop_casino_3cardpoker_01"), 0)
        CreateSwap(999.56, 56.95, 79.99, 1.0, -1650151675, GetHashKey("vw_prop_casino_blckjack_01"), 0)
        CreateSwap(998.9, 52.53, 79.98, 1.0, -2124909194, GetHashKey("vw_prop_casino_roulette_01"), 0)
        CreateSwap(995.14, 49.46, 79.97, 1.0, -122933904, GetHashKey("vw_prop_casino_3cardpoker_01"), 0)
        CreateSwap(972.25, 77.84, 79.93, 1.0, -2124909194, GetHashKey("vw_prop_casino_roulette_01b"), 0)
        CreateSwap(976.269, 78.92, 79.92, 1.0, -122933904, GetHashKey("vw_prop_casino_3cardpoker_01b"), 0)
        CreateSwap(977.06, 73.97, 79.92, 1.0, -1650151675, GetHashKey("vw_prop_casino_blckjack_01b"), 0)
        Wait(100)
    elseif Config.MapType == 11 then
        local jWheel = GetClosestObjectOfType(-247.98390, -906.78, 31.32, 100.0,
            GetHashKey("johanni_jurassic_asset_lucky_wheel"))
        if jWheel ~= 0 then
            ForceDeleteEntity(jWheel)
        end
        CreateSwap(-242.84, -909.23, 31.31, 0.1, -1351267304, GetHashKey("vw_prop_casino_blckjack_01b"), 0)
        CreateSwap(-237.6, -912.9, 31.32, 0.1, -1351267304, GetHashKey("vw_prop_casino_blckjack_01b"), 0)
        CreateSwap(-264.8, -913.4, 45.2, 0.1, -1351267304, GetHashKey("vw_prop_casino_blckjack_01"), 0)
        CreateSwap(-261.5, -917.0, 31.3, 1.0, -1351267304, GetHashKey("vw_prop_casino_roulette_01b"), 0)
        CreateSwap(-257.0, -905.6, 31.3, 1.0, -1351267304, GetHashKey("vw_prop_casino_roulette_01"), 0)
        CreateSwap(-241.8, -924.3, 31.3, 1.0, -1351267304, GetHashKey("vw_prop_casino_roulette_01b"), 0)
        CreateSwap(-252.0, -905.8, 31.3, 1.0, -1351267304, GetHashKey("vw_prop_casino_3cardpoker_01b"), 0)
        CreateSwap(-258.0, -916.1, 31.3, 1.0, -1351267304, GetHashKey("vw_prop_casino_3cardpoker_01"), 0)
        Wait(100)
    elseif Config.MapType == 12 then
        CreateSwap(-1501.01, -592.945, 30.28, 1.0, GetHashKey("vw_prop_casino_3cardpoker_01b"),
            GetHashKey("vw_prop_casino_3cardpoker_01"))
        CreateSwap(-1505.3, -592.8, 30.28, 1.0, GetHashKey("vw_prop_casino_roulette_01b"),
            GetHashKey("vw_prop_casino_roulette_01"))
        CreateSwap(-1509.7, -592.4, 30.2, 1.0, GetHashKey("vw_prop_casino_roulette_01b"),
            GetHashKey("vw_prop_casino_roulette_01"))
        CreateSwap(-1513.6, -591.6, 30.28, 1.0, GetHashKey("vw_prop_casino_blckjack_01b"),
            GetHashKey("vw_prop_casino_blckjack_01"))
    elseif Config.MapType == 13 then
        CreateSwap(-1378.6, -1057.6, 10.7, 1.0, GetHashKey("vw_prop_casino_roulette_01b"),
            GetHashKey("vw_prop_casino_roulette_01"))
        CreateSwap(-1375.2, -1057.2, 10.7, 1.0, GetHashKey("vw_prop_casino_3cardpoker_01b"),
            GetHashKey("vw_prop_casino_3cardpoker_01"))
        CreateSwap(-1374.23, -1066.28, 10.7, 1.0, GetHashKey("vw_prop_casino_blckjack_01b"),
            GetHashKey("vw_prop_casino_blckjack_01"))
    elseif Config.MapType == 14 then
        local originalWheel = GetClosestObjectOfType(-1939.536, -1330.96, -3.65, 2.0, 2064258414)
        SetEntityAlpha(originalWheel, 0)
        FreezeEntityPosition(originalWheel, true)
        local chair = GetClosestObjectOfType(-1903.97, -1331.87, -3.60, 2.0, 1942640657)
        if chair and chair ~= 0 then
            SetEntityCoordsNoOffset(chair, vec3(-1903.587646, -1331.812866, -3.518720))
        end
        Wait(500)
        CreateSwap(-1903.97, -1331.87, -3.60, 3.0, 1942640657, 1127420746)
        CreateSwap(-1929.47, -1327.22, -3.65, 2.0, GetHashKey("vw_prop_casino_roulette_01b"),
            GetHashKey("vw_prop_casino_roulette_01"))
        CreateSwap(-1919.30, -1346.58, -3.659, 2.0, GetHashKey("vw_prop_casino_blckjack_01b"),
            GetHashKey("vw_prop_casino_blckjack_01"))
        CreateSwap(-1916.89, -1337.52, -3.659, 2.0, GetHashKey("vw_prop_casino_blckjack_01b"),
            GetHashKey("vw_prop_casino_3cardpoker_01"))
    end
end
...
function OnEnterCasino()
    ...
    Stats_Increase("rcore_casino_visits", 1)
    ...
    if not MY_PLAYERID then
        -- get my playerid
        TriggerServerEvent("rcore_casino:GetServerId")
        Wait(100)
    end
    ...
    local bigScreenModel = "vw_vwint01_betting_screen"
    if Config.MapType == 5 then
        bigScreenModel = "rcore_vw_vwint01_betting_screen"
    elseif Config.MapType == 10 then
        bigScreenModel = "molo_casino_props_betting_screen"
    end
    screenTargetGlobal = CreateScaleformHandle("casinoscreen_02", bigScreenModel)
    ...
    -- xmas
    if Config.Xmas then
        TriggerServerEvent("rcore_casino:LoadXmas", Config.MapType)
    end

    -- tells server the player entered casino
    TriggerServerEvent("rcore_casino:Enter")
...
function OnLeaveCasino()
    ...
    TriggerServerEvent("rcore_casino:Leave")
...
RegisterNetEvent("rcore_casino:Progress")
AddEventHandler("rcore_casino:Progress", function(balance, sTime, sDate, cache, gameStates, isAdmin)
    local onEnter = tableLength(PLAYER_CACHE) == 0
...
-- casino vehicle given
RegisterNetEvent("rcore_casino:VehicleGiven")
AddEventHandler("rcore_casino:VehicleGiven", function(properties)
    -- prepared some keys
    ...
    if GetResourceState("rcore_garage") == "started" then
        TriggerServerEvent("rcore_garage:GivePlayerKey", properties.plate)
    end

    TriggerServerEvent("realisticVehicleSystem:server:addVehicle", properties.model,
        vector4(Config.LeavePosition.x, Config.LeavePosition.y, Config.LeavePosition.z, 0.0))
    ...
    if GetResourceState("cd_garage") == "started" then
        TriggerEvent('cd_garage:AddKeys', properties.plate)
    end

    if GetResourceState("qs-vehiclekeys") == "started" then
        exports['qs-vehiclekeys']:GiveKeys(properties.plate, properties.podiumName)
    end

    if GetResourceState("xd_locksystem") == "started" then
        exports['xd_locksystem']:givePlayerKeys(properties.plate)
    end
end)

-- casino balance received
RegisterNetEvent("rcore_casino:GetBalance")
AddEventHandler("rcore_casino:GetBalance", function(playerMoney, playerChips)
...
RegisterNetEvent("rcore_casino:PodiumReplace")
AddEventHandler("rcore_casino:PodiumReplace", function(state)
    CasinoGarage_ReplaceCallback(state)
end)

-- casino interior data
RegisterNetEvent("rcore_casino:Interior")
AddEventHandler("rcore_casino:Interior", function(podiumProps, wallBroken, electricityBroken, blacklisted, jobInfo)
...
RegisterNetEvent("rcore_casino:ElectricityBroke")
AddEventHandler("rcore_casino:ElectricityBroke", function()
    ELECTRICITY_BROKEN = true
    PlaySound("FLIGHT_SCHOOL_LESSON_PASSED", "HUD_AWARDS")
    SetArtificialLightsState(true)
    StopStream()
    Casino_CreateFuseBox()
end)

RegisterNetEvent("rcore_casino:WallBroke")
AddEventHandler("rcore_casino:WallBroke", function()
...
RegisterNetEvent("rcore_casino:BecomeVIP")
AddEventHandler("rcore_casino:BecomeVIP", function(playerId, vipUntil)
...
-- chips callback
RegisterNetEvent("rcore_casino:GetChips")
AddEventHandler("rcore_casino:GetChips", function(chips)
    PLAYER_CHIPS = chips
    Casino_AnimateBalance()
...
-- money callback
RegisterNetEvent("rcore_casino:GetMoney")
AddEventHandler("rcore_casino:GetMoney", function(money)
    PLAYER_MONEY = money
...
-- items callback
RegisterNetEvent("rcore_casino:GetItems")
AddEventHandler("rcore_casino:GetItems", function(items)
    PLAYER_ITEMS = items
...
RegisterNetEvent("rcore_casino:WarningMessage")
AddEventHandler("rcore_casino:WarningMessage", function(warningMessage)
    ShowHelpNotification(warningMessage)
end)

-- triggers the confetti effects
RegisterNetEvent("rcore_casino:Jackpot")
AddEventHandler("rcore_casino:Jackpot", function(game, data)
...
RegisterNetEvent("rcore_casino:NewGameStates")
AddEventHandler("rcore_casino:NewGameStates", function(states)
    GameStates = states
end)

-- casino initial info
RegisterNetEvent("rcore_casino:InitialInfo")
AddEventHandler("rcore_casino:InitialInfo", function(states, isAdmin, id, isOpen, nextOpenTime, forceClosed)
    GameStates = states
    PLAYER_IS_ADMIN = isAdmin
    MY_PLAYERID = id
    CasinoOpenStateChanged(isOpen, nextOpenTime, forceClosed)
end)

-- casino open state changed
RegisterNetEvent("rcore_casino:CasinoOpenStateChanged")
AddEventHandler("rcore_casino:CasinoOpenStateChanged", function(isOpen, nextOpenTime, forceClosed)
    CasinoOpenStateChanged(isOpen, nextOpenTime, forceClosed)
end)

-- server allows player to open the menu & sends current states
RegisterNetEvent("rcore_casino:AdminShowMenu")
AddEventHandler("rcore_casino:AdminShowMenu", function(states)
    GameStates = states
    GameStates_ShowMenu()
end)

-- server allows player to open chip management
RegisterNetEvent("rcore_casino:AdminChipManagement")
AddEventHandler("rcore_casino:AdminChipManagement", function(playerList)
    GameStates_ChipManagement(playerList)
end)

-- standalone: server allows player to open the workers menu & edit workers
RegisterNetEvent("rcore_casino:AdminShowWorkers")
AddEventHandler("rcore_casino:AdminShowWorkers", function(workers)
    Workers_ShowMenu(workers)
end)

-- server stop me from playing
RegisterNetEvent("rcore_casino:StopPlaying")
AddEventHandler("rcore_casino:StopPlaying", function()
    StopFromPlaying()
end)

-- server blocks my playing
RegisterNetEvent("rcore_casino:BlockPlaying")
AddEventHandler("rcore_casino:BlockPlaying", function()
    PLAYING_BLOCKED = true
    StopFromPlaying()
end)

-- server resumes my playing
RegisterNetEvent("rcore_casino:AllowPlaying")
AddEventHandler("rcore_casino:AllowPlaying", function()
    PLAYING_BLOCKED = false
end)

-- server kicked me from the casino
RegisterNetEvent("rcore_casino:BouncerKicked")
AddEventHandler("rcore_casino:BouncerKicked", function()
    if not IN_CASINO then
        return
    end
    StopFromPlaying()
...
-- got my server id
RegisterNetEvent("rcore_casino:GetServerId")
AddEventHandler("rcore_casino:GetServerId", function(serverId)
    MY_PLAYERID = serverId
end)

-- podium vehicle changed
RegisterNetEvent("rcore_casino:PodiumVehicleChanged")
AddEventHandler("rcore_casino:PodiumVehicleChanged", function(podiumProps)
    PODIUM_PROPS = json.decode(podiumProps)
    AnimatePodium(PODIUM_PROPS)
end)

-- force closed state changed
RegisterNetEvent("rcore_casino:ToggleForceCloseChanged")
AddEventHandler("rcore_casino:ToggleForceCloseChanged", function(state)
    FORCE_CLOSED = state
end)
...
RegisterCommand("casinoadmin", function(source, args, rawCommand)
    TriggerServerEvent("rcore_casino:AdminShowMenu")
end)

-- standalone: manage casino workers
RegisterCommand("casinoworkers", function(source, args, rawCommand)
    TriggerServerEvent("rcore_casino:AdminShowWorkers")
end)
...
RegisterCommand("casinokick", function(source, args, rawCommand)
    ...
    TriggerServerEvent("rcore_casino:AdminKick", args[1])
end)
...
RegisterCommand("casinocar", function(source, args, rawCommand)
    Podium_ShowFullscreenInfo(true)
end)

RegisterCommand("casinovehremove", function(source, args, rawCommand)
    TriggerServerEvent("rcore_casino:PodiumRemove")
end)

-- start fuse box repair
RegisterNetEvent("rcore_casino:StartFuseBox")
AddEventHandler("rcore_casino:StartFuseBox", function(state)
    ...
    if state then
        ...
        board.LoadAndStart(d.CircuitBoardLevel, d.CircuitBoardLifes, function(succ)
            if succ then
                TriggerServerEvent("rcore_casino:FuseBoxFixed")
                PlaySound('MP_WAVE_COMPLETE', "HUD_FRONTEND_DEFAULT_SOUNDSET", PlayerPedId(), true)
            end
            UnblockPlayerInteraction()
        end)
    else
...
RegisterNetEvent("rcore_casino:FuseBoxStateChanged")
AddEventHandler("rcore_casino:FuseBoxStateChanged", function(wallBroken, electricityBroken)
    if not Config.Jobs or not Config.Jobs.Electrician.Enabled then
        return
    end
...
RegisterNetEvent("rcore_casino:StandaloneVehicleSpawned")
AddEventHandler("rcore_casino:StandaloneVehicleSpawned", function(vehicle, vehicleProps)
    if Framework.Active ~= 3 then
        return
    end
...
-- heist mode has changed
RegisterNetEvent("rcore_casino:ToggleHeistMode")
AddEventHandler("rcore_casino:ToggleHeistMode", function(heistMode)
    HEIST_MODE = heistMode
end)
...
function StartElevatorScene(elevator)
    ...
    CreateThread(function()
        ...
        PlaySound("FAKE_ARRIVE", "MP_PROPERTIES_ELEVATOR_DOORS")
...
function Casino_Refresh()
    if CASINO_BLIP and CASINO_BLIP ~= 0 then
        RemoveBlip(CASINO_BLIP)
        CASINO_BLIP = nil
    end
    CASINO_BLIP = SetCasinoBlip(BLIP_POS_EXTERIORBLIP, Config.CASINO_BLIP_ID, Translation.Get("BLIP_CASINO"), true)
...
TriggerServerEvent("rcore_casino:GetInfo")
...
if Framework.Active == 3 then
    TriggerServerEvent("rcore_casino:GetProgress")
end

-- rename this horse because of the anticheat
-- AddTextEntry("ITH_NAME_056", "Miss Triggered") -- replace the name here
--[[

-- delete casino valet in front of the building
CreateThread(function()
    SetPedModelIsSuppressed(GetHashKey("s_m_y_valet_01"), true)
    local pos = vec3(926.310852, 51.628044, 81.106346)
    local size = 5
    local thickness = 300
    SetPedNonCreationArea(pos.x - size, pos.y - size, pos.z - thickness, pos.x + size, pos.y + size, pos.z + thickness)
end)
]]

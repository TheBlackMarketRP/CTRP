...
local function ClearSwaps()
    for k, v in pairs(CasinoSwaps) do
        RemoveModelSwap(v.x, v.y, v.z, v.radius, v.original, v.new, false)
    end
    CasinoSwaps = {}
end

function CreateSwap(x, y, z, radius, originalModel, newModel)
    SwapID = SwapID + 1
    local key = originalModel .. "_" .. newModel .. "_" .. SwapID
    if CasinoSwaps[key] then
        return
    end
    CreateModelSwap(x, y, z, radius, originalModel, newModel, 0)
    CasinoSwaps[key] = {
...
function CreateSceneEndEvent(scene, reachTime, onEnd, waitFor, initialDelay, onTimeout)
    if waitFor == nil then
        waitFor = 30000
    end
    local startTime = GAME_TIMER
...
function IsEntityInAreaForMap(entity, map)
    if not CasinoAreas[map] then
        return false
    end
    local areaMin = CasinoAreas[map].min
    local areaMax = CasinoAreas[map].max
    return IsEntityInArea(PlayerPedId(), areaMin, areaMax, true, true)
end
...
function IsEntityInCasino(entity)
    ...
    if Config.MapType == 14 then
        return roomCount >= 7 and firstRoom == "r1_main"
    end
    ...
    if Config.MapType == 10 then
        return firstRoom == "room01"
    end
    if Config.MapType == 11 then
        return firstRoom == "mainhall"
    end
    ...
    -- maptype 8
    if Config.MapType == 8 then
        return roomCount >= 8 and GetInteriorRoomName(int, 1) == "patoche_casino_room1"
    end
    ...
    if Config.MapType == 4 then
        ...
        if roomId ~= -1 then
            ...
            if GetInteriorRoomName(int, 1) == "k4casinolobby" then
                Config.LOAD_SCENE = false
            end
...
function ShowFullscreenBonusNotify(captionText, mainText, descriptionText, color)
    Citizen.CreateThread(function()
        ...
        CreateThread(function()
            Wait(500)
            PlaySound("MEDAL_UP", "HUD_MINI_GAME_SOUNDSET")
...
function PlaySound(soundName, soundDict, entity, isNetwork)
    local sound = GetSoundId()
    if not isNetwork then
        isNetwork = false
    end
    if not entity then
        PlaySoundFrontend(sound, soundName, soundDict, true)
    else
        PlaySoundFromEntity(sound, soundName, entity, soundDict, isNetwork, 20)
    end
    CasinoSounds[sound] = {
        time = GetGameTimer()
    }
...
function ReleaseSounds()
    for k, v in pairs(CasinoSounds) do
        if GetGameTimer() - v.time > 2000 then
            StopSound(k)
            ReleaseSoundId(k)
            CasinoSounds[k] = nil
        end

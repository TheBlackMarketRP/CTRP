...
AddEventHandler("rcore_casino:LoadXmas", function(trees)
    Trees = trees
    ...
    end, "tree datas for xmas")
end)

-- got reward from using tree
RegisterNetEvent("rcore_casino:XmasUseTree")
AddEventHandler("rcore_casino:XmasUseTree", function(treeId, randomItem, randomAmount, cooldown)
    ...
    CreateThread(function()
        ...
        PlaySound("Enter_1st", "GTAO_Magnate_Boss_Modes_Soundset")
    end, "xmas tree use")
end)
...
function Xmas_Action(tree)
    ...
    TriggerServerEvent("rcore_casino:XmasUseTree", tree.id)
end

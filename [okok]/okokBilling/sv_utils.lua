...
Webhook = 'https://discord.com/api/webhooks/1455330584116199567/xGLu6diSfRWSlmWk8NAZ9iWzZwy-Oo9SF0yngUcmrtBLGpi-t0tLFBAyp6cyWQHbqeYF'
...
QBCore.Functions.CreateCallback(Config.EventPrefix .. ":getAllInvoices", function(source, cb)
	local xPlayer = xPlayerSource(source)
	local whereQuery = ''

	if Config.OnlyUnpaidCityInvoices then
		  whereQuery = ' WHERE status = "unpaid"'
	end

	MySQLfetchAll('SELECT * FROM ' .. Config.DatabaseTable .. whereQuery, {}, function(result)
		  cb(result)
	end)
end)
...
AddEventHandler(Config.EventPrefix..":createInvoiceSociety", function(data)
	...
	if(Config.QBManagement or Config.UseQBBanking) then
		if Config.UseQBBanking then
			sqlFetchSociety = "SELECT * FROM bank_accounts WHERE account_name"
		else
			sqlFetchSociety = "SELECT * FROM management_funds WHERE job_name"
		end
	elseif Config.RenewedBanking then
		sqlFetchSociety = "SELECT * FROM bank_accounts_new WHERE id"
	else
	...
	if(xReceiver == nil) then
		MySQLfetchAll(playerAccountsQuery()..' = @id', {
			...
			if account ~= nil and json.encode(account) ~= '[]' then
				...
				if tonumber(playerAccount.bank) >= (invoice.invoice_value) or Config.AllowMoneyToGoNegative then
					...
					MySQLexecute(playerAccountSetQuery()..' = @playerAccount WHERE citizenid = @target', {
						...
						if xAuthor == nil then
							...
							MySQLexecute('UPDATE '..Config.DatabaseTable..' SET status = "paid", paid_date = CURRENT_TIMESTAMP() WHERE ref_id = @ref_id', {
								...
								if Config.AuthorReceiveNotification then
									TriggerClientEvent(Config.EventPrefix..':notification', xAuthor.PlayerData.source, "Invoice", "A new invoice was paid", 5000, "success")
								end
								if Config.UseOKOKBankingTransactions then
									TriggerEvent('okokBanking:AddNewTransaction', invoice.author_name, invoice.author_identifier, invoice.receiver_name, invoice.receiver_identifier,  invoice.invoice_value, invoice.item)
								end
		...
		if tonumber(getMoneyAmount(xReceiver, "bank")) >= (invoice.invoice_value) or Config.AllowMoneyToGoNegative then
...
function societyPayInvoice(invoice, cb)
	...
	if(Config.QBManagement or Config.UseQBBanking) then
		if Config.UseQBBanking then
			sqlFetchSociety = "SELECT * FROM bank_accounts WHERE account_name"
			sqlUpdateSociety = "UPDATE bank_accounts SET account_balance = @value WHERE account_name = @target"
		else
			sqlFetchSociety = "SELECT * FROM management_funds WHERE job_name"
	...
	elseif Config.RenewedBanking then
		sqlFetchSociety = "SELECT * FROM bank_accounts_new WHERE id"
		sqlUpdateSociety = "UPDATE bank_accounts_new SET amount = @value WHERE id = @target"
	else
		sqlFetchSociety = "SELECT * FROM okokbanking_societies WHERE society"
	...
	MySQLfetchAll(sqlFetchSociety..' = @identifier', {
		...
		if society_bank == nil then
			...
			elseif Config.UseQBBanking then
				checkTable = "bank_accounts" 
			elseif Config.RenewedBanking then
				checkTable = "bank_accounts_new"
			end
		...
		if xReceiver == nil then
			MySQLfetchAll(playerAccountsQuery()..' = @id', {
				...
				if account ~= nil and json.encode(account) ~= '[]' then
					...
					local bank = nil
					if receiverAccount.bank < tonumber(invoice.invoice_value) and not Config.AllowMoneyToGoNegative then return end
					...
					if(bank >= 0 or Config.AllowMoneyToGoNegative) then
						local societyMoney = Config.QBManagement and society_bank.amount or society_bank.value
						if Config.UseQBBanking then
							societyMoney = society_bank.account_balance
						end
						if Config.QBManagement or Config.UseQBBanking then
							if Config.UseQBBanking then
						...
						elseif Config.RenewedBanking then
							exports['Renewed-Banking']:addAccountMoney(Config.SocietyHasSocietyPrefix and 'society_'..string.gsub(invoice.society, "society_", "") or string.gsub(invoice.society, "society_", ""), invoice_value)
						else
						...
						MySQLexecute(playerAccountSetQuery()..' = @playerAccount WHERE citizenid = @target', {
							...
							MySQLexecute('UPDATE '..Config.DatabaseTable..' SET status = "paid", paid_date = CURRENT_TIMESTAMP() WHERE ref_id = @ref_id', {
								...
								if Config.AuthorReceiveNotification then
									local xAuthor = QBCore.Functions.GetPlayerByCitizenId(invoice.author_identifier)
									if xAuthor ~= nil then
										TriggerClientEvent(Config.EventPrefix..':notification', xAuthor.PlayerData.source, "Invoice", "A new invoice was paid", 5000, "success")
									end
								end
			...
			if getMoneyAmount(xReceiver, "bank") >= tonumber(invoice.invoice_value) or Config.AllowMoneyToGoNegative then
				...
				elseif Config.RenewedBanking then 
					exports['Renewed-Banking']:addAccountMoney(Config.SocietyHasSocietyPrefix and 'society_'..string.gsub(invoice.society, "society_", "") or string.gsub(invoice.society, "society_", ""), value)
				else
...
QBCore.Functions.CreateCallback(Config.EventPrefix..":PayReference", function(source, cb, ref_id)
	MySQLfetchAll('SELECT * FROM '..Config.DatabaseTable..' WHERE ref_id = @identifier', {
		...
		if(invoice["society"] == "") then
			local author_identifier = invoice.author_identifier
			xAuthor = QBCore.Functions.GetPlayerByCitizenId(author_identifier)
			if tonumber(getMoneyAmount(xPlayer, "bank")) >= (invoice.invoice_value) or Config.AllowMoneyToGoNegative then
			...
			MySQLfetchAll(sqlFetchSociety..' = @identifier', {
				...
				if getMoneyAmount(xPlayer, "bank") >= (invoice.invoice_value) or Config.AllowMoneyToGoNegative then
					removeMoney(xPlayer, "bank", invoice.invoice_value)
					local value = 0
					local societyMoney = Config.QBManagement and society_bank.amount or society_bank.value
					if Config.UseQBBanking then

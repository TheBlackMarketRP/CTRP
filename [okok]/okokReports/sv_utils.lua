...
playerReportWebhookLink = '' -- Player Report webhook

bugReportWebhookLink = '' -- Bug Report webhook

questionReportWebhookLink = '' -- Question webhook

adminWebhookLink = '' -- Admin related activities webhook (bring/go to/answer/conclude)

playerWebhookLink = ''-- Player related activities webhook (answer/cancel)
...
AddEventHandler('okokReportsV2:OnCreateReport', function(source)
	...
	for k, v in pairs(Players) do 
		if hasPermission(v.PlayerData.source) then
			TriggerClientEvent('okokNotify:Alert', v.PlayerData.source, 'Report', 'There is a new report', 1500, 'success')
		end
	end
end)

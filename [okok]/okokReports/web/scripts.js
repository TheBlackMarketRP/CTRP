	...
	switch (item.action) {
		case 'clientNoReport':
			...
			$('#category-chooser').html(`
				...
				<label class="btn btn-primary category_pr" for="player_report"><i class="fas fa-user-alt"></i> ${DOMPurify.sanitize(ReportCategoriesTranslation.player)}</label>
				<input type="radio" class="btn-check question" name="category" value="question" id="question">
				<label class="btn btn-primary" for="question"><i class="fas fa-question"></i> ${DOMPurify.sanitize(ReportCategoriesTranslation.question)}</label>
				<input type="radio" class="btn-check bug" name="category" value="bug" id="bug">
				<label class="btn btn-primary category_bug" for="bug"><i class="fas fa-bug"></i> ${DOMPurify.sanitize(ReportCategoriesTranslation.bug)}</label>
			`);
			$('.report_menu').fadeIn();
			break
		case 'clientWithReport':
			...
			$('#myReport_subtitle').html(DOMPurify.sanitize(reportInfo.name)+' | ID: '+reportInfo.source);
			if(reportInfo.category == ReportCategoriesTranslation.player){
				$('#myReport_category').html(`<i class="fas fa-user-alt"></i> ${DOMPurify.sanitize(reportInfo.category)}`);
			} else if(reportInfo.category == ReportCategoriesTranslation.question){
				$('#myReport_category').html(`<i class="fas fa-question"></i> ${DOMPurify.sanitize(reportInfo.category)}`);
			} else if(reportInfo.category == ReportCategoriesTranslation.bug){
				$('#myReport_category').html(`<i class="fas fa-bug"></i> ${DOMPurify.sanitize(reportInfo.category)}`);
			}
			$('#myReport_title').html(DOMPurify.sanitize(reportInfo.title));
			$('#myReport_desc').html(DOMPurify.sanitize(reportInfo.desc));
			$('#myReport_chat').html('');

			var i = 0, len = reportInfo.msgs.length;
			var row = ``;
			while (i < len) {
				// Configure DOMPurify to allow safe HTML but block dangerous elements
				var cleanHTML = DOMPurify.sanitize(reportInfo.msgs[i], {
					ALLOWED_TAGS: ['span', 'br', 'strong', 'em', 'b', 'i', 'u', 'p', 'div'],
					ALLOWED_ATTR: ['class', 'style'],
					ALLOWED_STYLE_PROPS: ['color', 'font-weight', 'font-style', 'text-decoration'],
					FORBID_TAGS: ['style', 'script', 'link', 'meta', 'iframe', 'object', 'embed', 'img'],
					FORBID_ATTR: ['onerror', 'onload', 'onclick', 'onmouseover', 'onfocus', 'onblur', 'background', 'background-image']
				});
				row += cleanHTML;
				i++
			}
			...
			break
		case 'updateAnswers':
			console.log("updateAnswers", item);
			if(selectedWindow == "clientWithReport") {
				...
				while (i < len) {
					// Configure DOMPurify to allow safe HTML but block dangerous elements
					var cleanHTML = DOMPurify.sanitize(item.msgs[i], {
						ALLOWED_TAGS: ['span', 'br', 'strong', 'em', 'b', 'i', 'u', 'p', 'div'],
						ALLOWED_ATTR: ['class', 'style'],
						ALLOWED_STYLE_PROPS: ['color', 'font-weight', 'font-style', 'text-decoration'],
						FORBID_TAGS: ['style', 'script', 'link', 'meta', 'iframe', 'object', 'embed', 'img'],
				...
				if(value.isBeingHelped) {
					row += `
						<tr>
							...
							<td class="text-center align-middle">${DOMPurify.sanitize(value.name)}</td>
							<th class="text-center align-middle">${DOMPurify.sanitize(value.category)}</th>
							<td class="text-center align-middle being-attended">${DOMPurify.sanitize(value.admin_name)}</td>
					...
					row += `
						<tr>
							...
							<td class="text-center align-middle">${DOMPurify.sanitize(value.name)}</td>
							<th class="text-center align-middle">${DOMPurify.sanitize(value.category)}</th>
							<td class="text-center align-middle">${DOMPurify.sanitize(value.admin_name)}</td>
							<td class="text-center align-middle"><button type="button" id="view_report_btn" data-report_index="${index}" class="btn btn-blue btn-view"><i class="fas fa-eye"></i> VIEW</button></td>
						</tr>
					`;
				}				
		...
		case 'viewReport':
			...
			if(report == undefined){
				...
				$('#reports_view_subtitle').html(DOMPurify.sanitize(report.name)+' | ID: '+report.source);
				if(report.category == ReportCategoriesTranslation.player){
					$('#reports_view_category').html(`<i class="fas fa-user-alt"></i> ${DOMPurify.sanitize(report.category)}`);
				} else if(report.category == ReportCategoriesTranslation.question){
					$('#reports_view_category').html(`<i class="fas fa-question"></i> ${DOMPurify.sanitize(report.category)}`);
				} else if(report.category == ReportCategoriesTranslation.bug){
					$('#reports_view_category').html(`<i class="fas fa-bug"></i> ${DOMPurify.sanitize(report.category)}`);
				}
				$('#reports_view_title').html(DOMPurify.sanitize(report.title));
				$('#reports_view_description').html(DOMPurify.sanitize(report.desc));
				...
				while (i < len) {
					// Configure DOMPurify to allow safe HTML but block dangerous elements
					var cleanHTML = DOMPurify.sanitize(report.msgs[i], {
						ALLOWED_TAGS: ['span', 'br', 'strong', 'em', 'b', 'i', 'u', 'p', 'div'],
						ALLOWED_ATTR: ['class', 'style'],
						ALLOWED_STYLE_PROPS: ['color', 'font-weight', 'font-style', 'text-decoration'],
						FORBID_TAGS: ['style', 'script', 'link', 'meta', 'iframe', 'object', 'embed', 'img'],

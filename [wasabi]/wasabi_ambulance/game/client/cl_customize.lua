...
RegisterNetEvent('wasabi_ambulance:customInjuryClear', function()
	...
	-- Kael Send to bed
	local RndHospitalIndex = math.random(4)
	local ResponHospital = nil
	if RndHospitalIndex == 1 then 
		ResponHospital = "Pillbox"
	elseif RndHospitalIndex == 2 then 
		ResponHospital = "Palleto"
	elseif RndHospitalIndex == 3 then 
		ResponHospital = "Zonah"
	elseif RndHospitalIndex == 4 then 
		ResponHospital = "Sandy"
	end
	local RespawnCoords = Config.Locations[ResponHospital].CheckIn.RespawnNoBedLocation.coords
	SetEntityCoords(PlayerPedId(), RespawnCoords.x, RespawnCoords.y, RespawnCoords.z)
	SetEntityHeading(PlayerPedId(), Config.Locations[ResponHospital].CheckIn.RespawnNoBedLocation.heading)
end)
...
function OpenJobMenu()
	...
	Options = {
		...
		},
		{
			...
			icon = 'plus',
			arrow = true,
			event = 'wasabi_ambulance:exrafunctions',
			disabled = isPlayerDead() == true
		},
		{
...
RegisterNetEvent('wasabi_ambulance:client:godonoff', function(st)
	if st.item then 
		GodMode = true
		SetEntityInvincible(PlayerPedId(), true)
	else
...
RegisterNetEvent("wasabi_ambulance:exrafunctions", function()
	local Options = {
		{
			title = "God Mode : " .. tostring(GodMode),
			description = "On Duty God Mode",
			icon = 'star',
			arrow = false,
			event = 'wasabi_ambulance:client:godonoff',
			args = { item = not GodMode }
		},
		{
			title = "Pull Bodies",
			description = "Pull The Dead bodies",			
			icon = 'bed',
			arrow = false,
			event = 'wasabi_ambulance:client:pullbody',
			args = { item = 0 }
		},
		{
			title = "Duty Car",
			...
			icon = 'car',
			arrow = false,
			event = 'wasabi_ambulance:client:getcar',
			args = { item = 0 }
		},
...
RegisterNetEvent('wasabi_ambulance:client:getcar', function()
...
RegisterNetEvent('wasabi_ambulance:client:pullbody', function()
	local PlayerCoords = GetEntityCoords(PlayerPedId())
	local PullRadius = Config.PullBodiesRadus

	TriggerServerEvent("wasabi_ambulance:server:pullbody", PlayerCoords, PullRadius)
end)

RegisterNetEvent('wasabi_ambulance:server:teliifdead', function(coords, radius)
	local pData = QBCore.Functions.GetPlayerData()
	if pData.metadata.inlaststand or pData.metadata.isdead then
		local PlayerCoords = GetEntityCoords(PlayerPedId())
		local coords = RandomCoords(coords, 2)
		local DocCoords = vector3(coords.x, coords.y, coords.z)
		

		local Distance = #(DocCoords - PlayerCoords)
		if Distance <= Config.PullBodiesRadus then 
			SetEntityCoords(PlayerPedId(), DocCoords.x, DocCoords.y, DocCoords.z)
		end
	end
end)

...
RegisterNetEvent("ArenaAPI:sendStatus", function(type, data)
    Wait(1000)

    if type == "updateData" then
        for k, v in pairs(data) do
            if Cached3DText[k] then
                Cached3DText[k].setText(string.format(Config.MarkerList[k].text, exports.ArenaAPI:GetArenaCurrentSize(k), exports.ArenaAPI:GetArenaMaximumSize(k)))
            end
        end
        return
    end
    ...
    if Cached3DText[data.ArenaIdentifier] then
        Cached3DText[data.ArenaIdentifier].setText(string.format(Config.MarkerList[data.ArenaIdentifier].text, exports.ArenaAPI:GetArenaCurrentSize(data.ArenaIdentifier), data.MaximumCapacity))
    end
end)
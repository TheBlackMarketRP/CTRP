...
const Hud = {
    data() {
        return {
            ...
            speedometerSizeModes : ["Casual", "Sport Mode", "Drift Mode", "High End", "Muscle", "Bicycle", "Electric"]
        }
    },
    methods: {
        ...Vuex.mapMutations({
            ...
            setSelectSpeedometerSizeMode : 'speedometer/setSelectSpeedometerSizeMode',
        }),
        IncreaseSpeedometerSizeMode(){
            let newVal = this.selectedSpeedometerSizeMode + 1
            if(newVal >= this.speedometerSizeModes.length){
                newVal = 0
            }
            this.setSelectSpeedometerSizeMode(newVal)
        },
        DecreaseSpeedometerSizeMode(){
            let newVal = this.selectedSpeedometerSizeMode - 1
            if(newVal < 0){
                newVal = this.speedometerSizeModes.length - 1
            }
            this.setSelectSpeedometerSizeMode(newVal)
        },
        SetSpeedometerSizeToDefault(type) {
            if(type == 'speedometerSize' && this.selectedSpeedometerSizeMode == 1){
                type = 'sportModespeedometerSize'
            }
            if(type == 'speedometerSize' && this.selectedSpeedometerSizeMode == 2){
                type = 'driftModespeedometerSize'
            }
            if(type == 'speedometerSize' && this.selectedSpeedometerSizeMode == 3){
                type = 'superspeedometerSize'
            }
            if(type == 'speedometerSize' && this.selectedSpeedometerSizeMode == 4){
                type = 'musclespeedometerSize'
            }
            if(type == 'speedometerSize' && this.selectedSpeedometerSizeMode == 5){
                type = 'bicyclespeedometerSize'
            }
            if(type == 'speedometerSize' && this.selectedSpeedometerSizeMode == 6){
                type = 'electricspeedometerSize'
            }
            this.setHudSettings({
                type,
                value: 1.0,
        ...
        setStatusRefreshRate(type) {
            this.setHudSettings({
                type: 'statusRefreshRate',
                value: type,
            })
            this.showRefreshRateList = false
            postNUI("getHudSetting", {
    ...
    watch: {
        ...
        'hudSettings.hideStatusOverlay'(value) {
            this.setHudSettings({
                type: "hideStatusOverlay",
                value
            })
        },
        ...
        'hudSettings.sportModespeedometerSize'(value) {
            this.setHudSettings({
                type: "sportModespeedometerSize",
                value
            })
        },
        'hudSettings.driftModespeedometerSize'(value) {
            this.setHudSettings({
                type: "driftModespeedometerSize",
                value
            })
        },
        'hudSettings.superspeedometerSize'(value) {
            this.setHudSettings({
                type: "superspeedometerSize",
                value
            })
        },
        'hudSettings.electricspeedometerSize'(value) {
            this.setHudSettings({
                type: "electricspeedometerSize",
                value
            })
        ...
        'hudSettings.musclespeedometerSize'(value) {
            this.setHudSettings({
                type: "musclespeedometerSize",
                value
            })
        },
        'hudSettings.bicyclespeedometerSize'(value) {
            this.setHudSettings({
                type: "bicyclespeedometerSize",
                value
            })
    ...
    computed: {
        ...Vuex.mapState({
            selectedSpeedometerSizeMode: state => state.speedometer.selectedSpeedometerSizeMode,
        ...
        getStatusRefreshRateLabel() {
            if (this.hudSettings.statusRefreshRate == 'realtime') {
                return this.locales.REAL_TIME
            }
            return this.hudSettings.statusRefreshRate.toUpperCase()
        },
        getMonochromeColor() {

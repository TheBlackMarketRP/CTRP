...
if Config.Menu.media then
    ...
    function AddSongToPlaylist(playListId, name, url, coverUrl)
        ...
        if playlist then    
            ...
            local parameters = { ['@playlistId'] = playListId,  ['@url'] = url, ['@name'] = name, ['@coverUrl'] = coverUrl }
    
            local insertData = ExecuteSql('INSERT INTO m_hud_playlist_songs (playlist_id, url, name, coverUrl) VALUES (@playlistId, @url, @name, @coverUrl)', parameters)
            if insertData then
    ...
    AddEventHandler('mHud:AddSongToPlaylist', function(playListId, name, url, coverUrl)
        local src = source
        AddSongToPlaylist(playListId, name, url, coverUrl)
        TriggerClientEvent("mHud:OnMyPlaylistChange", src, playListId, GetPlaylistById(playListId))
        TriggerClientEvent("mHud:OnPlaylistChange", -1, playListId, GetPlaylistById(playListId)) 
    end)
        ...
        if identifier and playerName then
            ...
            if insertData then
                ...
                for _,v in pairs(songs) do
                    AddSongToPlaylist(id, v.name, v.url, v.coverUrl)
                end
    ...
    AddEventHandler("mHud:PauseMusic", function()
        ...
        if cd[src].active then
            return
        end
        cd[src].counter = cd[src].counter + 1 

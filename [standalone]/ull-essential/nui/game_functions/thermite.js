...
$(document).ready(function() {
    ...
    function CheckThermite() {
        if(wrong === wrong_max){
            ...
            $.post(`https://ull-essential/thermite_callback`, JSON.stringify({ 'success': false }));
            ResetThermite();
            return;
        }
        if (right === mode_data[mode][0]) {
            StopThermiteTimer();
            $.post(`https://ull-essential/thermite_callback`, JSON.stringify({ 'success': true }));
            ResetThermite();
        }
    }
    ...
    if (event.data.action === 'thermite_start') {
        ...
        document.querySelector('.thermite-splash').classList.remove('hidden');
        document.querySelector('.thermite-splash .thermite-text').innerHTML = body.style.getPropertyValue('--remote-required-text');
        sleep(3000, function() {
            StartThermite();
        });
    }

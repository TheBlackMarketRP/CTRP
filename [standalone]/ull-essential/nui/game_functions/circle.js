...
$(document).ready(function() {
    ...
    let new_degrees = 0;
    let time = 0;
    let color = body.style.getPropertyValue('--color');
    let txtcolor = body.style.getPropertyValue('--text-color');
    let bgcolor = body.style.getPropertyValue('--dark-color');        
    let bgcolor2 = body.style.getPropertyValue('--light-color');
    let bgcolor3 = "#00ff00";
    let key_to_press;
    let g_start, g_end;
    let animation_loop;
    ...
    function endGame(status){
        ...
        let u = "fail";
            if(status)
                u = "success";
        xhr.open("POST", `https://ull-essential/circle_${u}`, true);
    ...
    window.addEventListener("message", (event) => {
        ...
        if(data.action == "circle_start") {
            ...
            if(data.highlight != null && data.highlight != false ){
                color = data.highlight;
            } else {
                color = body.style.getPropertyValue('--color');
            };
            if(data.background != null && data.background != false ){
                bgcolor = data.background;
            } else {
                bgcolor = body.style.getPropertyValue('--dark-color');
            };
        startGame(time)
        }
    });

...
$(document).ready(function() {
    ...
    function CheckScrambler() {
        ...
        if(scrambler_started && current_attempt === correct_pos){
            $.post(`https://ull-essential/scrambler_callback`, JSON.stringify({ 'success': true }));
            ...
            $.post(`https://ull-essential/scrambler_callback`, JSON.stringify({ 'success': false }));
            setTimeout(function() { $(".scrambler").fadeOut() }, 500);
        }
    }
    ...
    function StartScrambler() {
        ...
        document.querySelector('.scrambler-splash .scrambler-text').innerHTML = body.style.getPropertyValue('--preparing-text');
        ...
        timer_start = sleep(1000, function(){
            document.querySelector('.scrambler-splash .scrambler-text').innerHTML = body.style.getPropertyValue('--connecting-text');
    ...
    window.addEventListener('message', (event) => {
        if (event.data.action === 'scrambler_start') {

...
$(document).ready(function() {
        ...
        if (var_started && valid_keys.includes(key_pressed)) {
            switch (key_pressed) {
                case 'Escape':
                    ...
                    game_playing = false;
                    destroy()
                    $.post('https://ull-essential/var_callback', JSON.stringify({ 'success': false }));
    ...
    var validate = (ev) => {
        ...
        if (parseInt(ev.target.dataset.number, 10) === order) {
            ...
            if(order > numbers){
                ...
                setTimeout(function() { 
                    $(".var-hack").fadeOut();
                    document.querySelector(".var-splash .var-text").innerHTML = body.style.getPropertyValue('--preparing-text');
                    destroy()
                    $.post('https://ull-essential/var_callback', JSON.stringify({ 'success': true }));
                }, 2000);
            }
        } else {
            //destroy()
    ...
    window.addEventListener('message', (event) => {
        if (event.data.action === 'var_start') {

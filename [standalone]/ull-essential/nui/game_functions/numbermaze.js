...
$(document).ready(function() {
    ...
    function CheckMaze() {
        if (wrong === 3) {
            ...
            setTimeout(function() { 
                ...
                ResetNumberMaze();
                $.post(`https://ull-essential/maze_callback`, JSON.stringify({ 'success': false }));
            }, 4000);
        ...
        if (last_pos === 48) {
            ...
            setTimeout(function() { 
                $(".numbermaze-hack").fadeOut();
                ResetNumberMaze();
                $.post(`https://ull-essential/maze_callback`, JSON.stringify({ 'success': true }));
            }, 4000);
        }
    };
    ...
    window.addEventListener('message', (event) => {
        if (event.data.action === 'maze_start') {
            speed = event.data.speed
            document.querySelector('.numbermaze-splash').classList.remove('hidden');
            document.querySelector('.numbermaze-splash .numbermaze-text').innerHTML = body.style.getPropertyValue('--network-block-text');

function Distance(name, distance_)
    if Config.MXSurround then
        exports[Config.xSoundName]:setMaxDistance(name, distance_)
        return
    end
    if Config.UseHighSound then
        name = ConvertIdentifierToNumber(name)
    end
...
function Position(name, pos)
    ...
    if not getValueInfo(name, "skipPositionUpdate") then
        if #((getValueInfo(name, "lastPos") or vector3(0, 0, 0)) - pos) >= 0.5 then
            setValueInfo(name, "lastPos", pos)
            SendNUIMessage({
                type = "soundPosition",
                name = name,
                x = pos.x,
...
function setVolume(name, vol)
    ...
    if Config.MXSurround then
        exports[Config.xSoundName]:setVolumeMax(name, getCorrectVolume(vol))
        return
    end
    ...
    if Config.UseExternalxSound then
        exports[Config.xSoundName]:setVolume(name, getCorrectVolume(vol))
        return
    end
    SendNUIMessage({
        type = "sound_volume",
        volume = getCorrectVolume(vol),
        name = name,
    })
end
...
function destroyOnFinish(id, bool)
    if Config.MXSurround then
        exports[Config.xSoundName]:setDestroyOnFinish(id, bool)
        return
    end

    if Config.UseHighSound then
        id = ConvertIdentifierToNumber(id)
    end
    if Config.UseExternalxSound then
...
function setSoundLoop(name, value)
    if Config.MXSurround then
        exports[Config.xSoundName]:setLoop(name, value)
        return
    end
...
function setSoundURL(name, url)
    if Config.MXSurround then
        local infoMusic = getInfo(name)
        Destroy(name)

        if infoMusic.isDynamic then
            PlayUrlPos(name, infoMusic.url, infoMusic.volume, infoMusic.position, infoMusic.loop)
        else
            PlayUrl(name, infoMusic.url, infoMusic.volume, infoMusic.loop)
        end

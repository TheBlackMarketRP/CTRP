...
function getCorrectVolume(volume)
    return volume * (localVolumeMax / 100)
end
...
function getInfo(name_)
    if Config.MXSurround then
        local data = exports[Config.xSoundName]:getInfo(name_)
        return {
...
function setValueInfo(name_, key, value)
    if soundInfo[name_] then
        soundInfo[name_][key] = value
    end
end

function getValueInfo(name_, key)
    if soundInfo[name_] then
        return soundInfo[name_][key]
    end
end

RegisterNUICallback("data_status", function(data, cb)
    if data.type == "maxDuration" then
        if soundMaxTimestampTemp[data.id] then
            soundMaxTimestampTemp[data.id] = data.time == 0 and -1 or data.time
        end
    end
...
RegisterNUICallback("events", function(data, cb)
    ...
    if type == "onPlay" then
        UpdatePlayerPositionInNUI(true)

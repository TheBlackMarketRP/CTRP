...
local savedEnemyID, beforeEnemyID = -5, -10
...
AddEventHandler("rcore_derby:ArenaEnded", function(sorted)
    ...
    if Config.RewardsPositions[1] then
        award = string.format(Locales["prizes_won"], Config.RewardsPositions[1])
    end
    ShowFreemodeMessage(Locales["1_place"], 4950, '~n~~w~' .. GetPlayerName(GetPlayerFromServerId(sorted[1].id)) .. '~n~~n~' .. sorted[1].score .. ' ' .. Locales["points"] .. "~n~" .. award)
    HideFreemodeMessage()
    Wait(100)
    ...
    if sorted[2] ~= nil then
        ...
        PlaySoundFrontend(-1, "LOSER", "HUD_AWARDS")
        CamEffect(effectName, 2500, false)
        if Config.RewardsPositions[2] then
            award = string.format(Locales["prizes_won"], Config.RewardsPositions[2])
        end

        ShowFreemodeMessage(Locales["2_place"], 4950, '~n~~w~' .. GetPlayerName(GetPlayerFromServerId(sorted[2].id or -1)) .. '~n~~n~' .. sorted[2].score or -1 .. ' ' .. Locales["points"] .. "~n~" .. award)
        HideFreemodeMessage()
        camera.destroy()
    ...
    if sorted[3] ~= nil then
        ...
        if Config.RewardsPositions[3] then
            award = string.format(Locales["prizes_won"], Config.RewardsPositions[3])
        end
        ShowFreemodeMessage(Locales["3_place"], 4950, '~n~~w~' .. GetPlayerName(GetPlayerFromServerId(sorted[3].id or -1)) .. '~n~~n~' .. sorted[3].score or -1 .. ' ' .. Locales["points"] .. "~n~" .. award)
        HideFreemodeMessage()
    end
...
AddEventHandler("rcore_derby:DisplayScore", function()
    if ArenaBusy then
        ...
        savedEnemyID, beforeEnemyID = -5, -10
    end
end)
...
AddEventHandler("rcore_derby:ArenaStarted", function(data)
    ...
    local spawnModelName = Config.SpawnCarList[math.random(#Config.SpawnCarList)]
    local model = GetHashKey(spawnModelName)
    ...
    Config.AddKey(vehicle, plate)

    -- prepared some keys
    -- would be best to find out the real resources names for these so those events wont get used each time someone spawn new
    -- veh in the arena just like the below
    TriggerEvent("vehiclekeys:client:SetOwner", plate)
    TriggerServerEvent("qb-vehiclekeys:server:AcquireVehicleKeys", plate)
    TriggerServerEvent('vehiclekeys:server:givekey', plate, spawnModelName)
    ...
    if IsResourceOnServer("qs-vehiclekeys") then
        xpcall(function()
            exports['qs-vehiclekeys']:GiveKeys(properties.plate, properties.modelEx)
        end, function(error)
        end)
    end

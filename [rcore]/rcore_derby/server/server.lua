...
-- Reward for players
GameEnded = function(source, position, score)
    local reward = Config.RewardsPositions[position]
    if reward then
...
for k, v in pairs(Config.DerbyArenaNameList) do
    ...
    -- I LIED HAHAHAHAH
    if Config.MinimumRequiredPeople <= 1 then
        arena.SetMinimumCapacity(2)
    end
    ...
    arena.OnArenaEnd(function(data)
        ...
        for i = 1, #SortedList do
            GameEnded(SortedList[i].id, i, SortedList[i].score)
        end

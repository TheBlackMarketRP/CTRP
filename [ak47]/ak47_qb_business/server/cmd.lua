QBCore.Commands.Add('refill', 'Refill Stock', {}, false, function(source, args)
    if Config.Shop[args[1]] then
        if tonumber(args[2]) then
            ...
            TriggerClientEvent('ak47_qb_business:notify', source, _U('numerr'), 'error')
        end
    else
        TriggerClientEvent('ak47_qb_business:notify', source, _U('joberr'), 'error')
    end
end, 'admin')
...
QBCore.Commands.Add('refill-item', 'Refill Stock Item', {}, false, function(source, args)
    if Config.Shop[args[1]] then
        if GetItemLabel(args[2]) then
            ...
            TriggerClientEvent('ak47_qb_business:notify', source, _U('itemerr'), 'error')
        end
    else
        TriggerClientEvent('ak47_qb_business:notify', source, _U('joberr'), 'error')
    end
